...
if (isRingoJS()) {
    ...
    function wrapRouteGet(app, ringoPath, hapiPath, props, commonHandler, errorHandler) {
        ...
        if (props.length > 0 && props.indexOf('fmt') >= 0) {
            var lbracePos = hapiPath.indexOf('{');
            var pathPrefix = hapiPath.slice(0, lbracePos - 1);
            var pathElements = pathPrefix.split('/');
            var group = '';
            var item = ' ';
            if (pathElements.length === 2) {
                ...
                group = pathElements[1];
                item = pathElements[2];
            }
            var itemCap = '/' + item[0].toUpperCase() + item.slice(1);

            var params = {};
            console.log('# hapiPath', hapiPath, group, item, props);
            props.forEach(function(i) {
                var p;
                if (i === 'id') {
                    p = joi.string().description('The id for the entity').required();
                }
                else if (i === 'fmt') {
                    p = joi.string().description('The output format for the result').default('json');
                }
                else {
                    p = joi.string().description('Additional parameter "' + i + '"');
                }
            ...
            Object.assign(config,
                            {
                                description: 'Get Information about (' + item + ') by: ' + props,
                                notes: 'Get Information about (' + item + ') by: ' + props,
                                tags: ['api'],
                                validate: {
                                    params: params
                                }
