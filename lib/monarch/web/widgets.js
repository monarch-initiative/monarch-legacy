...
function genTableOfAnnotateTextResults(results) {
    console.log("#Results="+results.length);
    return genTable(
        {
            columns: [
                ...
                {name: "term",
                 val: function(a){ return genObjectHref(a.entity.category, a.entity) + " (" + a.entity.id + ")" }
                },
            ]
        },
        results);
...
function genTableOfGeneDiseaseAssociations(assocs) {
    return genTable({
        ...
        columns: [
            ...
            {name: "onset",
             val: function(a){ return convChars(a.disease.onset) } // TODO - should be an object
            },
            {name: "frequency",
             val: function(a){ return convChars(a.disease.frequency) }
			},makeReferencesFromAssociations(assocs),
            {name: "source",
             val: function(a){ return genSourceHref('source', a.source) }
            }
...
function genListOfGeneReferences(assocs){
	var test = {};
	test = makeReferencesFromAssociations(assocs);
	console.log("Print gene test foo object: ");
