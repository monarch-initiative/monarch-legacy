...
function genTable(spec, rows) {
    ...
    if (rows != null) {
        ...
        for (var j = 0; j < spec.columns.length; j++) {
            ...
            content += "<th data-sort='" + datatype + "'>" + colname + "</th>\n";
        }
        content += "</tr>\n</thead>\n<tbody>\n";
...
function tableSortDataType(name) {
    var string_types = ["allele", "association type", "disease", "disease A", "disease B",
        "evidence", "gene", "gene A", "gene B", "gene B organism", "genotype", "hit",
        "homolog", "homology class", "inferred from", "interaction detection method",
        "interaction type", "model", "model species", "model type", "most informative shared phenotype",
        "mutation", "onset", "organism", "other matching phenotypes", "pathway", "phenotype",
        "phenotype description", "qualifier", "reference", "references", "relationship",
        "variant"];
    var float_types = ["combined score", "phenotype similarity score", "rank"];
...
function genKEGGShowPathwayLink(pathway,label,gene_ko_ids) {
	console.log('label:'+label+" ids:"+gene_ko_ids+'pathway:'+JSON.stringify(pathway));
	//KEGG map ids == KEGG ko pathway ids, which have an associated kgml file                                    
	//so convert the id here
	var koPathId = pathway.id.replace(/path:map/,"ko");
	...
	var gene_string = JSON.stringify(gene_ko_ids);
	if (gene_string != null) {
		gene_string.replace(/\"/g,'\'');
	}
	var keggeratorAnnotation = 
		"{ " + "pathways: ['" + koPathId + "'], " +
		"phenotypes: [ " +
		"{ name: '" + convChars(label) + "', " +
		"genes: " +gene_string + "}" +
		"]" +
		"}" ;
	var link = 
	"[<a href='#' onClick=\"keggerator.init();keggerator.annotate(" + 
	keggeratorAnnotation + ")\" >show</a>] "+pathway.label ;
	return link;
}
