...
function genTableOfGenePhenotypeAssociations(assocs) {
    console.log("#GeneAssocs="+assocs.length);
    return genTable({
        columns: [
            {name: "gene",
            val: function(a){ return genObjectHref('gene', a.gene) }
            ...
            {name: "inferred from",
            val: function(a){
//                 if (a.disease != null || a.disease !== "undefined") {
                    return convChars('disease-phenotype association');
                // } else if (a.has_genotype != null || a.has_genotype !== "undefined") {
...
function genTableOfGeneAlleleAssociations (assocs) {
    return genTable({
        ...
        columns: [
            ...
            {name: "type",
            	val: function(a){ return convChars(a.type) } //convChars(a.variant.variant_type.type)  }
            },
            {name: "evidence",
                val: function(a){ return genExternalHref('references', a.references) }
            },
...
function genTableOfSimilarPapers(assocs) {
    console.log("#SimilarPapers=" + assocs.length);
...
function genTableOfLiteratureGenes(assocs) {
    console.log("#Genes=" + assocs.length);
    return genTable(
        {
            columns: [
                {name: "species",
...
function sourceImage(source) {
    ...
    } else if (source.match(/FB|FlyBase/)) {
    ...
    } else if (source.match(/WB|WormBase/)) {
