            ...
            columns: [
                        ...
                        if (a.pathways != null) {
                            for (var i = 0; i < a.pathways.length; i++) {
                                ...
                                **/
                                ...
                                pathResult = pathResult +
                                    "[<a href='#' onClick=\"keggerator.init();keggerator.annotate(" + keggeratorAnnotation + ")\" >show</a>] "+pathwayLabel+"<br />"
                ...
                {name: "genes",
                 val: function (a) {
					var geneResult = "";
                        if (a.gene_ids){
                        //TODO Assuming gene ids and symbols are in the same order! //HACK
						for (var i = 0; i < a.gene_ids.length; i++) {
							//HARDCODE - resource does not prefix with NCBIGene yet.
							//TODO need to fix upstream
							if (!(a.gene_ids[i].match(/^NCBI/))) {
								a.gene_ids[i] = 'NCBIGene:'+a.gene_ids[i];
							}
                            geneResult = geneResult + " "+
								genObjectHref('gene',{id:a.gene_ids[i],label:a.gene_symbols[i]});
                        }
                        return geneResult;
                        }
				...
				{name: "references",
				 val: function(a) {return a.references.map(
						function(i) {return genListOfExternalIdentifierLinks({id:i})})}
				},
                {name: "source",
                     val: function(a){ return genSourceHref('source', a.source) }
                }
