...
function genObjectHref(type,obj,fmt) {
    ...
    if (env.getEnv().USE_SPA) {
        return '<a data-monarch-legacy href="'+url+'">'+label+'</a>';
    }
    else {
...
function genExternalHrefObject(type,obj,fmt) {
    if (obj == null) {
        return {};
    }
    if (obj.type != null && obj.type.id != null) {
        return genExternalHrefObject(type, obj.type, fmt);
    }
    if (obj.map != null) {
        return obj.map(function(x){return genExternalHrefObject(type,x,fmt)});
    }
...
function genExternalHref(type,obj,fmt) {
    ...
    var url = '';
    console.log('###obj', type, obj, obj.url, obj.id);
    if (typeof obj.url != 'undefined' && obj.url != null && obj.url != '') {
        url = obj.url;
    } else {
        url = makeExternalURL(obj.id);
...
function makeExternalURL(id) {
    ...
    if (url != "" && id != null) {
        ...
        // else {
        //     url = '/resolve/' + id;
        //     console.log('makeExternalURL resolve', id, url );
        // }
    }
...
//        image = '<img class="source" src="../image/source-omia.png"/>';
        ...
        image = '<img class="source" src="../image/source-string.png"/>';
    } else if (source.match(/Bgee/)) {
        image = '<img class="source" src="../image/source-bgee.png"/>';
    } else if (source.match(/RGD/)) {
        image = '<img class="source" src="../image/source-rgd.gif"/>';
    } else if (source.match(/SGD/)) {
        image = '<img class="source" src="../image/source-sgd.png"/>';
    }
    return image;
}
...
exports.genExternalHrefObject = genExternalHrefObject;
