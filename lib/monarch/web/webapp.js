...
if (useBundle) {
    var bundledCSSLibs = [];
    var bundledJSLibs = [];
    bundledJSLibs.push('/dist/app.bundle.js');
    if (!useWebpack) {
        bundledCSSLibs.push('/dist/app.bundle.css');
    }
    pup_tent.set_common('css_libs', bundledCSSLibs);
    pup_tent.set_common('js_libs', bundledJSLibs);
}
else {
    ...
    //Configure pup tent common js libs
...
function _decide_content_encoding(ctype){
    var encoding = 'utf8';
    if (ctype === 'image/png') {
        encoding = 'binary';
    }
          ...
          app.route({
            ...
            handler:
              function (request, reply) {
                ...
                // console.log('get(', thing, ') ', ctype, ' length:', body.length);
                var response = reply(body);
                response.type(ctype);
                var encoding = _decide_content_encoding(ctype);
...
web.wrapRouteGet(app, '/fonts/:f', '/fonts/{f}', ['f'],
    function(request, f) {
        // OLD: var path = './fonts/'+f;
        var path = './node_modules/bootstrap/dist/fonts/'+f;
        var ctype = _decide_content_type(path);
...
function searchHandler(request, term, fmt) {
    ...
    try {
        ...
        info.monarch_launchable.push('search_results_init(globalSearchTerm)');
...
function neurosearchHandler(request, term, fmt) {
    try {
        ...
        // adorn object with rendering functions
        info.genResultsTable = function() {
            return genTableOfSearchDataResults(info.otherResults);
        };
...
// Order matters here in the declaration of these routes.
web.wrapRouteGet(app, '/neurosearch/:term.:fmt?', '/neurosearch/{term}.{fmt}', ['term', 'fmt'], neurosearchHandler, errorResponse);
web.wrapRouteGet(app, '/neurosearch/:term', '/neurosearch/{term}', ['term'], neurosearchHandler, errorResponse);
...
web.wrapRouteGet(app, '/genotype', '/genotype', [], modelLandingPageHandler, errorResponse);
...
function addGeneTableWithFilter() {
    ...
    return expandTemplate('golr-table-filter', gene_table);
}
...
function addVariantTableWithFilter() {
    var variant_table = {href: "variants", div: "variant-table"};
    return expandTemplate('golr-table-filter', variant_table);
}
...
function addGenotypeTableWithFilter() {
    var genotype_table = {href: "genotypes", div: "genotype-table"};
    return expandTemplate('golr-table-filter', genotype_table);
}
