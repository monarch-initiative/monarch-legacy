...
app.get('/docs/*', function(request) {
    ...
    console.log("serving direct.. "+path+", "+ct);
...
app.get('/labs/gene', function(request) {
    var info = prepLandingPage();
    info.blog_results = loadBlogData('gene-news', 4);
    info.spotlight = engine.fetchSpotlight('gene');
    info.spotlight.link = genObjectHref('gene',{id:info.spotlight.id, label:"Explore"});
    ...
    var output = pup_tent.render('gene_main-test.mustache', info);
    var res =  response.html(output);
    return res;
});
...
// Ping for exomiser-core test
app.get('/exomiser-ping.:fmt?',function(request,fmt) {
    var info = engine.pingExomiser();
    if (fmt != null) {
        return formattedResults(info,fmt,request);
    }
    else 
	{
	    var val = "<HTML><BODY>count "+info.count+"</body></html>";
	    return response.html(val);
	}
});

app.get('/exomiser-test.:fmt?',function(request,fmt) {
    var vcfPath = module.resolve("../../../docs/Pfeiffer.vcf");
    info = {}
    info["path"] = vcfPath;
    if (fmt != null) {
	return formattedResults(info,fmt,request);
    ...
    {
	var val = "<HTML><BODY>count "+info.path+"</body></html>";
	return response.html(val);
    }
});
