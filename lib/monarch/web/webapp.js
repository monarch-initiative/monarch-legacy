...
var pup_tent_dirs = [
                    ...
                    'node_modules/isotope-layout/dist', // used by search results - Zhou
...
function searchFilteringHandler(request, term, category, species) {
    console.log('searchHandler: term: ', term);
    console.log('searchHandler: category: ', category);
    console.log('searchHandler: species: ', species);
    try {
       ...
       // }
        ...
        // Only display the first 1000 results per page start from row 1
        ...
        // Note: jeremy will add support of species soon
        var reqSpecies = [];
        if (typeof(species) !== 'undefined' && species !== 'all') {
            reqSpecies.push(species);
        }

        // TODO new implementation for Zhou
        var golrResults = engine.search(term, reqCategories, ["category", "leaf", "taxon_label"], resultsCount, 1);
...
/*        info.results.forEach( function (i) {
        ...
        if (golrResults.response.docs.length > 0) {
            var count = golrResults.response.docs.length;
            ...
            resultsOutput = '<table class="search-results-table table table-striped table-condensed simpletable">'
                + '<thead><tr><th width="40%">Term</th><th width="20%">Category</th><th>Taxon</th></tr></thead>'
                + '<tbody>';

            var docs = golrResults.response.docs;
            
            for (var i = 0; i < docs.length; i++) {
                // Temp workaround since some results don't have taxon info - Zhou
                var taxonText;

                if (typeof(docs[i].taxon_label) !== 'undefined') {
                    taxonText = docs[i].taxon_label;
                } else {
                    taxonText = 'Not specified';
                } 

                // Only use category[1], exclude Class, and sequence feature - Zhou
                // Don't forget the space to sprarate multi CSS classes
                resultsOutput += '<tr class="search-result-item">'
                    + '<td>' + genObjectHref(docs[i].category, docs[i]) +'</td>'
                    + '<td>' + docs[i].category[1] + '</td>'
                    + '<td>' + taxonText + '</td>'
                    + '</tr>';
            }
...
web.wrapRouteGet(app, '/searchfiltering/:term/:category/:species', '/searchfiltering/{term}/{category}/{species}', ['term', 'category', 'species'], searchFilteringHandler, errorResponse);
...
function searchHandler(request, term) {
    ...
    try {
       ...
       // }
        ...
        // TODO new implementation for Zhou
        var golrResults = engine.search(term, [], ["category", "leaf", "taxon_label"], resultsCount, 0);
...
/*        info.results.forEach( function (i) {
        ...
        // adorn object with rendering functions
        if (info.results.response.docs.length > 0) {
            // The total number of matching results found
            info.count = info.results.response.docs.length;

            info.hasResults = true;

            // Not using table to render search results - Zhou
            //info.resultsTable = function() {return genTableOfSearchResults(info.results); };
            
            // Combination filters
            // sort in alphabetical and ascending order
            var taxons = info.results.facet_counts.facet_fields.taxon_label.sort();
            console.log(taxons);

            // sort in alphabetical and ascending order
            var categories = info.results.facet_counts.facet_fields.category.sort();

            // Category filter
            info.resultsCategoryFilter = '<div class="search-results-button-group js-radio-button-group" data-filter-group="category">'
                + '<button class="search-results-button is-checked" data-filter="">All Categories</a>';
            for (var k = 0; k < categories.length; k++) {
                // Must have dot . to prefix the class in data-filter
                info.resultsCategoryFilter += '<button class="search-results-button" data-filter="' + categories[k][0] + '">' + categories[k][0] + '(' + categories[k][1] + ')</button>';
            }
            info.resultsCategoryFilter += '</div>';

            // Species filter
            // If possible that some results don't have taxon specified - Zhou
            info.resultsSpeciesFilter = '<div class="search-results-button-group js-radio-button-group" data-filter-group="species">'
                + '<button class="search-results-button is-checked" data-filter="">All Species</button>'
                + '<button class="search-results-button" data-filter=".not_specified">Not specified</button>';
            for (var j = 0; j < taxons.length; j++) {
                // Must have dot . to prefix the class in data-filter
                // convert space separateed taxon name into underscore separated CSS class - Zhou
                var taxonClassSelector = taxons[j][0].split(' ').join('_').toLowerCase();
                info.resultsSpeciesFilter += '<button class="search-results-button" data-filter="' + taxons[j][0] + '">' + taxons[j][0] + '(' + taxons[j][1] + ')</button>';
            }
            info.resultsSpeciesFilter += '</div>';
            ...
            var docs = info.results.response.docs;
            ...
            info.resultsOutput = ''; // "<span class=\"no-results\">&nbsp;&nbsp;No results found</span>";
...
function addNonEmptyAnchors(info, inhibit_filters_kludge) {
    ...
    if (info.orthologNum && info.orthologNum > 0) {
...
function literatureByIdHandler(request, id, fmt) {
    ...
    var assoc_filter = [{ field: 'subject_category', value: 'publication', plist : ['-'] }];

    addGolrTable(info, "source", id, 'association-table', assoc_filter, 'generic_association','#associations');
    info.assocNum = engine.fetchAssociationCount(id, 'source', assoc_filter);
...
function addGolrTable(info, query_field, id, div, filter, personality, anchor) {
    ...
    var is_leaf_var = "is_leaf_" + suffix;
    var launch = '';
    ...
    info.pup_tent_js_variables.push({name: is_leaf_var, value: info.isLeafNode});
...
function diseaseByIdHandler(request, id, fmt) {
    ...
    var literature_filter = [{ field: 'subject_category', value: 'publication' },
                         { field: 'object_category', value: 'disease' }];
    addGolrTable(info, "object_closure", id, 'literature-table', literature_filter, 'lit_disease_disp','#literature');
    info.literatureNum = engine.fetchAssociationCount(id, 'object_closure', literature_filter, 'subject');
...
function geneByIdHandler(request, id, fmt) {
    try {
        ...
        var ortho_phenotype_filter = [{ field: 'object_category', value: 'phenotype' }];
        addGolrTable(info, "subject_ortholog_closure", info.cliqueLeader, 'ortholog-phenotype-table',
                ortho_phenotype_filter, 'gene_phenotype','#ortholog-phenotypes');

        info.orthoPhenoNum = engine.fetchAssociationCount(info.cliqueLeader, 'subject_ortholog_closure', ortho_phenotype_filter, 'object');
