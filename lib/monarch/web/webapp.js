    ...
    try {
        ...
        if (golrResults.response.docs.length > 0) {
            ...
            for (var i = 0; i < highlighted_docs.length; i++) {
                ...
                // Exclude "Class" and "sequence feature"
                var modifiedCategories = highlighted_docs[i].category.filter(function(val){
                    if (val !== 'Class' && val !== 'sequence feature') {
                        return true;
                    }
                ...
                var categoryArr = modifiedCategories.map(function(val) {
    ...
    try {
       ...
       // }
        ...
        if (info.results.response.docs.length > 0) {
            ...
            var taxa = info.results.facet_counts.facet_fields.taxon_label.sort();
            //console.log(taxa);
            ...
            for (var k = 0; k < categories.length; k++) {
                // Exclude Class and sequence feature - Zhou
                if (categories[k][0] !== 'Class' && categories[k][0] !== 'sequence feature') {
            ...
            for (var j = 0; j < taxa.length; j++) {
                info.resultsSpeciesFilter += '<button class="search-results-button" data-filter="' + taxa[j][0] + '">' + taxa[j][0] + '(' + taxa[j][1] + ')</button>';
            }
