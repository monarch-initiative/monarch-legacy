...
function addGolrScratchInfo(info, query_field, id) {
    ...
    info.pup_tent_js_variables.push({name:'queryID', value: id});
    info.pup_tent_js_variables.push({name:'query_field', value: query_field});
    ...
    info.monarch_launchable = [];
    info.monarch_launchable.push('getTableFromSolr(queryID, query_field);');
...
app.get('/labs/widget-scratch',
    function(request, page){
        ...
        addGolrScratchInfo(info, "subject_closure", id);
...
app.get('/labs/widget-scratch/phenotype/:id.:fmt?',
        function(request, id, fmt){
            ...
            addGolrScratchInfo(info, "object_closure", id);
...
app.get('/labs/widget-scratch/disease/:id.:fmt?',
        ...
        try {
            ...
            engine.fetchAssociations(id, 'subject_closure', filter, 50000); //test, delete if still here
            engine.log("getting /disease/:id where id="+id);
            var newId = engine.resolveClassId(id);
            if (newId != id && typeof newId != 'undefined' ) {
                engine.log("redirecting: "+id+" ==> "+newId);
...
//Example: /disease/DOID_12798/phenotype_associations.json
//Currently only works for json or rdf output
app.get('/labs/widget/disease/:id/:section.:fmt?', function(request, id, section, fmt) {
 var newId = engine.resolveClassId(id);
 if (newId != id) {
     engine.log("redirecting: "+id+" ==> "+newId);
     return response.redirect(genURL('disease',newId));
