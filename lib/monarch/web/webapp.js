...
var fetchGenotypePage = function(request, id, fmt) {
    try {
		...
		info.has_part.forEach(function(g) { if (g.type == 'intrinsic_genotype') { intrinsic_genotype= g } else if (g.type = 'extrinsic_genotype') { extrinsic_genotype = g }} );
		var gvc = {};
		if (intrinsic_genotype != {} && typeof intrinsic_genotype.has_part != 'undefined') {
			gvc = intrinsic_genotype.has_part.map(function(g) { if (g.type == 'genomic_variation_complement') { return g } } );
		}
		//build variant/gene associations
		var vassoc = [];
		for (var i=0; i<Object.keys(gvc).length; i++) {
			var a = {};
			a.variant = {};
			a.variation_type = {};
			a.gene = {};
		...
		info.hasVariants = function() { return checkExistence(vassoc)};
        info.hasSim = function() {return checkExistence(info.sim)};
        info.hasLiterature = function() {return checkExistence(info.literature)};
        
        //info.phenotypeNum = function() {return getNumLabel(info.phenotype_associations)};
		console.log("ASSOC: "+JSON.stringify(info.phenotype_associations));
	    info.phenotypeNum = function() {
		    if (typeof info.phenotype_associations != 'undefined' && info.phenotype_associations != {} && info.phenotype_associations != null) {
			    return getNumLabel(engine.unique(info.phenotype_associations.map(function(p) {return p.has_phenotype.type.id})))};
			return 0;
		};
...
app.get('/labs/blog-scratch',
	function(request, page){
	    ...
	    // Grab off page.
	    var rsrc = 
		'http://monarch-initiative.blogspot.com/feeds/posts/default?alt=rss';
	    var res = [];
	    try {
		var http_client = require("ringo/httpclient");
		var exchange = http_client.get(rsrc);
		if( exchange && exchange.content ){
		    var got = exchange.content;
		    //console.log('got: ' + got);

		    // For E4X bug 336551.
		    got = got.replace(/^<\?xml\s+version\s*=\s*(["'])[^\1]+\1[^?]*\?>/, "");
		    var rss = new XML(got);
