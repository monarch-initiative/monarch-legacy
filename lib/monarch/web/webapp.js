...
function searchFilteringHandler(request, term, filters, pageNum) {
    ...
    try {
        ...
        var golrResults = engine.search(term, JSON.parse(filters), ["category", "leaf", "taxon_label"], resultsCountPerPage, (pageNum - 1)*resultsCountPerPage);

        // object with rendering functions
        if (golrResults.response.docs.length > 0) {
            var count = golrResults.response.numFound;
            ...
            resultsOutput = '<table class="search-results-table table table-striped table-condensed simpletable">'
                + '<thead><tr><th width="25%">Term</th><th width="25%">Category</th><th>Taxon</th><th>Matching String</th></tr></thead>'
                + '<tbody>';
...
var bgr = new bbop_response_golr(golrResults);
            ...
            for (var i = 0; i < docs.length; i++) {
                ...
                resultsOutput += '<tr class="search-result-item">'
                    ...
                    + '<td>' + taxonText + '</td>'
                    + '<td>' + bgr.get_doc_highlight(docs[i].id, 'label_searchable', term) + '</td>'
                    + '</tr>';
            }
