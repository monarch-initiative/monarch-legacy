...
//Resolve ID to appropriate page
 app.get('/resolve/:id.:fmt?',
         function(request, id, fmt){
             ...
             if (metadata && metadata['category']){
                 ...
                 if (variant_categories.indexOf(category) > -1){
                     return response.redirect(genURL('variant', id));
                 } else {
                     return response.redirect(genURL(category.toLowerCase(), id));
                 }
             } else {
 ...
 app.get('/labs/phenotype/:id.:fmt?',
         function(request, id, fmt){
             ...
             info.hasGenes = true;
             info.pup_tent_js_variables.push({name:'globalID',value:id});
             info.monarch_launchable.push("getOntologyBrowser(globalID)");
