...
function addCoreRenderers(info, type, id) {
    ...
    if (info.relationships != null) {
        ...
        info.hasEquivalentClasses = (equivalentClasses.length > 0);
        info.hasSubClasses = (subClasses.length > 0);
        info.hasSuperClasses = (subClasses.length > 0);
        info.ontNavBox = function(){ return genOntologyGraphInfo(type, id, info.relationships); };
        info.superClassTable = function(){ return genTableOfClasses(type, id, superClasses); };
        info.subClassTable = function(){ return genTableOfClasses(type, id, subClasses); };
        info.equivalentClassTable = function(){ return genTableOfClasses(type, id, equivalentClasses); };
    }
    info.includes = {};
    info.includes.navbar = Mustache.to_html(getTemplate('navbar'), {});
    info.includes.navlist = Mustache.to_html(getTemplate('navlist'), {});
    info.includes.navlistgenotype = Mustache.to_html(getTemplate('navlist_genotype'), {});
...
app.get('/disease/:id.:fmt?', function(request, id, fmt) {
    try {
        ...
        info.ids = function() {
            if (info.database_cross_reference != null && info.database_cross_reference.length > 0) {
                var num = 1;
                var result = info.database_cross_reference[0];
                while (num != info.database_cross_reference.length) {
                    result += ", ";
                    result += info.database_cross_reference[num];
                    num += 1;
                }
                return result;
            }
        ...
        info.hasPhenotypes = (info.phenotype_associations.length > 0);
        info.hasGenes = (info.gene_associations.length > 0);
        info.hasAlleles = (info.alleles.length > 0);
        info.hasModels = (info.models.length > 0);
        info.hasSim = (info.sim.length > 0);
        info.hasPathways = (info.pathways.length > 0);
...
app.get('/phenotype/:id.:fmt?', function(request, id, fmt) {
    try {
        ...
        info.hasDiseases = (info.disease_associations.length > 0);
        info.hasGenotypes = (info.genotype_associations.length > 0);
