...
function literatureByIdHandler(request, id, fmt) {
    ...
    addGolrTable(info, "subject_closure", id, 'gene-table', gene_filter, 'literature_gene','#genes');
    info.geneNum = engine.fetchAssociationCount(id, 'subject_closure', gene_filter, 'object');

    var genotype_filter = [{ field: 'object_category', value: 'genotype' },
                           { field: 'subject_category', value: 'publication' }];
    addGolrTable(info, "subject_closure", id, 'genotype-table', genotype_filter, 'literature_genotype',"#genotypes");
    ...
    var model_filter = [{ field: 'object_category', value: 'model' },
                        { field: 'subject_category', value: 'publication' }];
    addGolrTable(info, "subject_closure", id, 'model-table', model_filter, 'literature_model','#models');
    info.modelNum = engine.fetchAssociationCount(id, 'subject_closure', model_filter, 'object');

    var variant_filter = [{ field: 'object_category', value: 'variant' },
                          { field: 'subject_category', value: 'publication' }];
    addGolrTable(info, "subject_closure", id, 'variant-table', variant_filter, 'literature_variant','#variants');
    info.variantNum = engine.fetchAssociationCount(id, 'subject_closure', variant_filter, 'object');

    var phenotype_filter = [{ field: 'object_category', value: 'phenotype' },
                          { field: 'subject_category', value: 'publication' }];
    addGolrTable(info, "subject_closure", id, 'phenotype-table', phenotype_filter, 'literature_phenotype','#phenotypes');
    info.phenotypeNum = engine.fetchAssociationCount(id, 'subject_closure', phenotype_filter, 'object');
    
    var disease_filter = [{ field: 'object_category', value: 'disease' },
                            { field: 'subject_category', value: 'publication' }];
    addGolrTable(info, "subject_closure", id, 'disease-table', disease_filter, 'literature_disease','#diseases');
    info.diseaseNum = engine.fetchAssociationCount(id, 'subject_closure', disease_filter, 'object');
    ...
    // Add disease table
    info.includes.disease_anchor = addDiseaseAnchor(info);
    info.includes.disease_table = addDiseaseTableWithFilter();

    var literatureLauncher = 'fetchLiteratureOverview(' + regres[1] + ')';
    info.monarch_launchable.push(literatureLauncher);

    info.title = 'Monarch Literature: '+info.journalTitle+' ('+ info.id+')';
...
function diseaseByIdHandler(request, id, fmt) {
    ...
    info.publicationNum = engine.fetchAssociationCount(id, 'object_closure', literature_filter, 'subject');
