...
function buildEngine(defaultConfig, golrConfig) {
    ...
    engine.cache = {
        ...
        sizeInfo: function () {
            var info =
                this.cacheDirs().map(function (dir) {
                    return {
                        id: dir,
                        entries: env.fs_listTreeSync("cache/" + dir).filter(function (f) {
        ...
        contents: function () {
            return env.fs_listTreeSync("cache").filter(function (f) {
                return f.indexOf(".json") > 0
            });
        }
    };
...
function addCoreRenderers(info, type, id, defaults) {
    ...
    info.pup_tent_js_variables.push({
        name: 'global_app_base',
        value: engine.config.app_base
    });
    info.pup_tent_js_variables.push({
        name: 'global_scigraph_url',
        value: engine.config.scigraph_url
    });
    info.pup_tent_js_variables.push({
        name: 'global_scigraph_data_url',
        value: engine.config.scigraph_data_url
    });
    info.pup_tent_js_variables.push({
        name: 'global_solr_url',
        value: engine.config.golr_url
    });
    info.pup_tent_js_variables.push({
        name: 'global_golr_conf',
    ...
    if (info.relationships != null) {
        ...
        var node = equivalentClasses.map(function (c) {
            return c.id;
        }).concat(id);
        ...
        info.superClasses = superClasses.map(function (c) {
            return genObjectHref(type, c);
        });
        info.subClasses = subClasses.map(function (c) {
            return genObjectHref(type, c);
        });
        ...
        uniqueList.forEach(function (i) {
            var tmpObj = {};
            tmpObj['id'] = i;
            var nodes = equivalentClasses.filter(function (v) {
                return v.id === i
            });
        ...
        info.equivalentClasses = uniqueObjList.map(function (c) {
            return genObjectHref(type, c) + " (" + c.id + ")";
        });
    }
    info.includes = {};
...
function blankNodeHandler(request) {
    ...
    addCoreRenderers(info);
    info.pup_tent_css_libraries.push("/tour.css");
    var output = pup_tent.render('blank-node.mustache', info, 'monarch_base.mustache');
    return web.wrapHTML(output);
}
...
function diseaseLandingHandler(request) {
    ...
    info.spotlight.heritability = info.spotlight.heritability.map(function (h) {
        return h.label
    }).join(", ");

    var phenotypes = _sortByLabel(info.spotlight.phenotypes).map(function (p) {
        return genObjectHref('phenotype', p);
    });
    ...
    if (info.spotlight.genes != null && info.spotlight.genes.length > 0) {
        ...
        genes = info.spotlight.genes.map(function (p) {
            return genObjectHref('gene', p);
        });
    }
...
function phenotypeLandingHandler(request) {
    ...
    var genes = _sortByLabel(info.spotlight.genes).map(function (g) {
        return genObjectHref('gene', g);
    });
    info.spotlight.genes = genes.slice(0, 5).join(", ");
    if (genes.length > 5) {
        info.spotlight.genes += ", <span class=\"toggleitems\"><span class=\"fewitems\"> [and " + (genes.length - 5) + " more...]</span><span class=\"moreitems\">";
    ...
    var diseases = _sortByLabel(info.spotlight.diseases).map(function (p) {
        return genObjectHref('disease', p);
    });
...
function modelByIdHandler(request, id, fmt) {
    try {
        ...
        info.primary_xref = function () {
            return genExternalHref('source', {id: id})
        };
        info.xrefs = function () {
            return buildXrefsList(id, info.database_cross_reference);
        };
        info.hasXrefs = function () {
            return checkExistence(info.database_cross_reference) || !!info.primary_xref
        };

        if (info.taxon) {
            info.taxon_xref = function () {
                return genExternalHref('source', info.taxon)
            };
        }
        ...
        if (info.equivalentClasses) {
            info.equal_ids = function () {
                return info.equivalentClasses.map(function (r) {
                    return genObjectHref('model', {id: r, label: r})
                }).join(", ");
            }
        }
        ...
        info.hasDef = function () {
            return checkExistence(info.definitions)
        };
        info.hasComment = function () {
            return checkExistence(info.comment)
        };
        info.hasAka = function () {
            return checkExistence(info.synonyms)
        };
        info.hasEqual = function () {
            return checkExistence(info.equivalentClasses)
        };
...
function genotypeByIdHandler(request, id, fmt) {
    try {
        ...
        if (info.equivalentClasses) {
            info.equal_ids = function () {
                return info.equivalentClasses.map(function (r) {
                    return genObjectHref('genotype', {id: r, label: r})
                }).join(", ");
            }
        }
...
function geneLandingHandler(request) {
    ...
    var pathways = info.spotlight.pathways.map(function (p) {
        p.label
    });
    info.spotlight.pathways = pathways.slice(0, 5).join(", ");
    if (pathways.length > 5) {
...
function annotateTextHandler(request) {
    ...
    info.resultsTable = function () {
        return genTableOfAnnotateTextResults(info.results, info.query);
    };
...
function annotateMinimalTextHandler(request, fmt) {
    ...
    info.resultsTable = function () {
        return genTableOfAnnotateTextResults(info.results, 'obopurl');
    };
...
if (false) {
// Should be deleted or implemented as a proper feature
...
function diseaseByIdHandler(request, id, fmt) {
    ...
    info.hasHeritability = function () {
        return checkExistence(info.inheritance)
    };
    info.hasModifier = function () {
        return checkExistence(info.modifier)
    };
    info.hasPhenotypes = info.phenotypeNum;
...
function geneByIdHandler(request, id, fmt) {
    try {
        ...
        if (info.equivalentClasses) {
            info.equal_ids = function () {
                return info.equivalentClasses.map(function (r) {
                    return genObjectHref('gene', {id: r, label: r})
                }).join(", ");
            }
        }
...
function variantByIdHandler(request, id, fmt) {
    try {
        ...
        if (info.equivalentClasses) {
            info.equal_ids = function () {
                return info.equivalentClasses.map(function (r) {
                    return genObjectHref('variant', {id: r, label: r})
                }).join(", ");
            }
        }
        ...
        info.hasTypes = function () {
            return checkExistence(info.categories)
        };
        info.hasEqual = function () {
            return checkExistence(info.equivalentClasses)
        };
...
function addPhenotypeTable(info) {
    var phenotype_table = {
        href: "phenotypes", div: "phenotypes-table",
...
function addLiteratureAnchor(info) {
    var literature_anchor = {
        id: info.id, type: "Literature",
...
function analyzeByDatatypeGetHandler(request, datatype, fmt) {
    ...
    info.results = info.results.filter(function (a) {
        return a.combinedScore > 0;
    });
    info.resultsTable = function () {
