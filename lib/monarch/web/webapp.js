...
app.get('/gene/:id.:fmt?', function(request, id, fmt) {
	...
	info.primary_xref = function() {return genExternalHref('source',{id : id})};
	info.xrefs = function() {return genExternalHref('source',info)};
	
	//Link out to NCBI for taxonomy, hardcoded
	//need to update with standard way of creating xrefs
    info.tax_xref;
	
	if (info.tax_xref){
		info.tax_xref = "<a href=\"https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?id="
			             + info.taxon.id + "\">" + info.taxon.id + "</a>";
		//info.tax_xref = function() {return genExternalHref('source',info.tax_xref)};
	}
