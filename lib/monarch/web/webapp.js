...
function searchHandler(request, term) {
    ...
    try {
       ...
       // }
        ...
        //var results = engine.search(term, [], ["category", "leaf", "taxon_label"], resultsCount, 1);
...
/*        info.results.forEach( function (i) {
        ...
        if (info.results.length > 0) {
            ...
            var groupsByTaxon = _.groupBy(info.results, "taxon");
            // sort in alphabetical and ascending order
            var taxons = Object.keys(groupsByTaxon).sort();
            console.log(taxons);

            // group the results by category
            var groupsByCategory = _.groupBy(info.results, "category");
            // sort in alphabetical and ascending order
            var categories = Object.keys(groupsByCategory).sort();
            ...
            for (var j = 0; j < taxons.length; j++) {
                ...
                if (taxons[j] === '') {
                    ...
                    var taxonClassSelector = taxons[j].split(' ').join('_').toLowerCase();
                    info.resultsSpeciesFilter += '<button class="search-results-button" data-filter=".' + taxonClassSelector + '">' + taxons[j] + '</button>';
                }
            ...
            for (var k = 0; k < categories.length; k++) {
                info.resultsCategoryFilter += '<button class="search-results-button" data-filter=".' + categories[k] + '">' + categories[k] + '</button>';
            }
            ...
            for (var i = 0; i < info.results.length; i++) {
                // convert space separateed taxon name into underscore separated CSS class - Zhou
                if (info.results[i].taxon === '') {
                    var taxonSelector = 'not_specified';
                } else {
                    var taxonSelector = info.results[i].taxon.split(' ').join('_').toLowerCase();
                }
                // Don't forget the space to sprarate multi CSS classes
                info.resultsOutput += '<div class="search-result-box ' + taxonSelector + ' ' + info.results[i].category + '">'
                    + '<ul>'
                    + '<li>Term: ' + genObjectHref(info.results[i].category, info.results[i]) +'</li>'
                    + '<li>Category: ' + info.results[i].category + '</li>'
                    + '<li>Taxon: ' + info.results[i].taxon + '</li>'
                    + '<li>Matching String: ' + info.results[i].completion + '</li>'
                    + '</ul>'
                    + '</div>';
            }
        } else {
        ...
        // Include underscore before search_results.js
        info.pup_tent_js_libraries.push("/underscore-min.js");
