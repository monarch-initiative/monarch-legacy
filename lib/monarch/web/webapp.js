...
function caseByIdHandler(request, id, fmt) {
    try {
        ...
        var phenotype_filter = [{ field: 'object_category', value: 'phenotype' }];
        addGolrTable(info, "subject_closure", id, 'phenotypes-table', phenotype_filter, 'individual_phenotype', '#phenotypes');
        info.phenotypeNum = engine.fetchAssociationCount(id, 'subject_closure', phenotype_filter, 'object');

        var gene_filter = [{ field: 'object_category', value: 'gene' }];
        addGolrTable(info, "subject_closure", id, 'gene-table', gene_filter, 'individual_gene', '#genes');
        info.geneNum = engine.fetchAssociationCount(id, 'subject_closure', gene_filter, 'object');

        var variant_filter = [{ field: 'object_category', value: 'variant' }];
        addGolrTable(info, "subject_closure", id, 'variant-table', variant_filter, 'individual_variant','#variants');
        info.variantNum = engine.fetchAssociationCount(id, 'subject_closure', variant_filter, 'object');
        ...
        var output = pup_tent.render('case.mustache', info,
                                     'monarch_base.mustache');
        return web.wrapHTML(output);
    }
    catch(err) {
...
function literatureByIdHandler(request, id, fmt) {
    ...
    info.includes.association_table = addAssociationTableWithFilter();
...
function addDiseaseTableWithFilter() {
    var disease_table = {href: "diseases", div: "disease-table", filter: true};
    return expandTemplate('golr-table', disease_table);
}
...
function addAssociationTableWithFilter() {
    var gene_table = {href: "associations", div: "association-table", filter: true}
    return expandTemplate('golr-table', gene_table);
}
...
function addOrthoPhenoAnchor(info) {
    var ortho_pheno_anchor = {id: info.id,
                            ...
                            type: "Homolog-Phenotypes", href: "ortholog-phenotypes"};
