        ...
        if (golrResults.response.docs.length > 0) {
            ...
            for (var i = 0; i < highlighted_docs.length; i++) {
                ...
                resultsOutput += '<tr class="search-result-item">'
                    + '<td>' + genObjectHref(highlighted_docs[i].category, highlighted_docs[i]) +'</td>'
                    + '<td>' + highlighted_docs[i].category.join(', ') + '</td>'
                    + '<td>' + taxonText + '</td>'
                    + '<td>' + highlighted_docs[i].label_searchable + '</td>'
                    + '</tr>';
            }
...
function searchHandler(request, term) {
    ...
    try {
       ...
       // }
        ...
        // TODO new implementation for Zhou
        var golrResults = engine.search(term, {}, ["category", "leaf", "taxon_label"], resultsCountPerPage, 0);
