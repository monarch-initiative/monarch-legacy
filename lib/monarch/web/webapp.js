...
function searchFilteringHandler(request, term, filters) {
    ...
    try {
        ...
        var golrResults = engine.search(term, JSON.parse(filters), ["category", "leaf", "taxon_label"], resultsCount, 0);
        ...
        if (golrResults.response.docs.length > 0) {
            ...
            resultsOutput = '<table class="search-results-table table table-striped table-condensed simpletable">'
                + '<thead><tr><th width="25%">Term</th><th width="25%">Category</th><th>Taxon</th></tr></thead>'
                + '<tbody>';
...
web.wrapRouteGet(app, '/searchfiltering/:term/:filters', '/searchfiltering/{term}/{filters}', ['term', 'filters'], searchFilteringHandler, errorResponse);
...
/*        info.results.forEach( function (i) {
        ...
        if (info.results.response.docs.length > 0) {
            ...
            info.resultsSpeciesFilter = '<div class="search-results-button-group js-radio-button-group" data-filter-group="taxon_label">'
