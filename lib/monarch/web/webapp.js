...
var pup_tent_loader = require('../../../modules/pup-tent');
...
//Configure pup tent common css libs
...
if (useBundle) {
    pup_tent.set_common('js_libs', ['/app.bundle.js']);
}
else {
...
// Order matters here in the declaration of these routes.
web.wrapRouteGet(app, '/sources.:fmt?', '/sources.{fmt}', ['fmt'], sourcesHandler, errorResponse);
web.wrapRouteGet(app, '/sources', '/sources', [], sourcesHandler, errorResponse);
...
function fetchFeatureSection(request, id, section, fmt) {
    var info = engine.fetchSectionInfo(id);
    ...
    if (node) {
        ...
        if (metadata && metadata['category']) {
            ...
            if (variant_categories.indexOf(category) > -1){
                returnFunc = web.wrapRedirect(genURL('variant', id));
            }
            else {
                returnFunc =  web.wrapRedirect(genURL(category.toLowerCase(), id));
            }
        } else {
            console.log(id);
            // See if we're a model
            if (/^(Coriell|FBst|JAX)/.test(id)) {
                returnFunc = web.wrapRedirect(genURL('model', id));
            } else {
