...
function addNonEmptyAnchors(info, inhibit_filters_kludge) {
    ...
    if (info.diseaseCausalNum && info.diseaseCausalNum > 0) {
        info.includes.disease_causal_anchor = addDiseaseCausalAnchor(info);
        info.includes.disease_causal_table = addDiseaseCausalTable();
    }
    if (info.diseaseOtherNum && info.diseaseOtherNum > 0) {
        info.includes.disease_other_anchor = addDiseaseOtherAnchor(info);
        info.includes.disease_other_table = addDiseaseOtherTable();
    }
    if (info.geneCausalNum && info.geneCausalNum > 0) {
        info.includes.gene_causal_anchor = addGeneCausalAnchor(info);
        info.includes.gene_causal_table = addGeneCausalTable();
    }
    if (info.geneOtherNum && info.geneOtherNum > 0) {
        info.includes.gene_other_anchor = addGeneOtherAnchor(info);
        info.includes.gene_other_table = addGeneOtherTable();
    }
}
...
function addGolrTable(info, query_field, div, filter, personality,
                      anchor, is_static_id, or_filter, is_symmetric, query_id) {
    var replacer = new RegExp('-', 'g');
    ...
    var is_symmetric_var = "is_symmetric_" + suffix;
    var query_id_var = "query_id_" + suffix;
    var launch = '';
    ...
    info.pup_tent_js_variables.push({name: is_symmetric_var, value: is_symmetric});
    info.pup_tent_js_variables.push({name: query_id_var, value: query_id});
...
function diseaseByIdHandler(request, id, fmt) {
    ...
    var gene_filter = [
        { field: 'subject_category', value: 'gene' },
        { field: 'object_category', value: 'disease' },
        { field: 'association_type', value: 'gene_disease' },
        { field: 'object_closure', value: id }
    ];
    addGolrTable(info, 'object_closure', 'gene-causal-table', gene_filter, 'gene_disease_causal','#gene-causal');
    info.geneCausalNum = engine.fetchAssociationCount(gene_filter, 'subject');

    var gene_other_filter = [
        { field: 'object_category', value: 'disease' },
        { field: 'subject_category', value: 'gene' },
        { field: 'association_type', value: 'marker_disease' },
        { field: 'object_closure', value: id }
    ];
    addGolrTable(info, 'object_closure', 'gene-other-table', gene_other_filter, 'gene_disease','#gene-other');
    info.geneOtherNum = engine.fetchAssociationCount(gene_other_filter, 'subject');
...
function geneByIdHandler(request, id, fmt) {
  console.log('geneByIdHandler', id, fmt);
    try {
        ...
        addGolrTable(info, 'subject_closure', 'disease-causal-table', disease_filter, 'gene_disease_causal','#disease-causal');
        info.diseaseCausalNum = engine.fetchAssociationCount(disease_filter, 'object');

        var disease_other_filter = [
            { field: 'object_category', value: 'disease' },
            { field: 'subject_category', value: 'gene' },
            { field: 'association_type', value: 'marker_disease' },
            { field: 'subject_closure', value: id }
        ];
        addGolrTable(info, 'subject_closure', 'disease-other-table', disease_other_filter, 'gene_disease','#disease-other');
        info.diseaseOtherNum = engine.fetchAssociationCount(disease_other_filter, 'object');
        ...
        var homolog_or_filter = [
            { field: 'subject_closure', value: id },
            { field: 'object_closure', value: id }
        ]
        addGolrTable(info, 'subject_closure', 'homolog-table', homolog_filter,
            'gene_homolog','#homologs', false, homolog_or_filter, isSymmetric, id);
        info.homologNum = engine.fetchAssociationCount(homolog_filter, null, homolog_or_filter);
        ...
        var interaction_or_filter = [
            { field: 'subject_closure',  value: id},
            { field: 'object_closure',  value: id}
        ];

        addGolrTable(info, 'subject_closure', 'interaction-table',
                interaction_filter, 'gene_interactions', '#interactions', false, interaction_or_filter, isSymmetric, id);
...
function addAnatomyTable() {
 ...
 function addDiseaseCausalAnchor(info) {
     const disease_anchor = {
         id: info.id,
         resultNum: info.diseaseCausalNum,
         type: 'Diseases, Causal',
         href: 'disease-causal'
     };
     return expandTemplate('anchor', disease_anchor);
 }

 function addDiseaseCausalTable() {
     const disease_table = {
         href: 'disease-causal', div: 'disease-causal-table', filter: false
     };
     return expandTemplate('golr-table', disease_table);
 }

 function addGeneCausalAnchor(info) {
     const gene_anchor = {
         id: info.id,
         resultNum: info.geneCausalNum,
         type: 'Genes, Causal',
         href: 'gene-causal'
     };
     return expandTemplate('anchor', gene_anchor);
 ...
 function addGeneCausalTable() {
     const gene_table = {
         href: 'gene-causal', div: 'gene-causal-table', filter: false
     };
     return expandTemplate('golr-table', gene_table);
 }

 function addDiseaseOtherAnchor(info) {
     const disease_anchor = {
         id: info.id,
         resultNum: info.diseaseOtherNum,
         type: 'Diseases, Other',
         href: 'disease-other'
     };
     return expandTemplate('anchor', disease_anchor);
 ...
 function addDiseaseOtherTable() {
     const disease_table = {
         href: 'disease-other', div: 'disease-other-table', filter: false
     };
     return expandTemplate('golr-table', disease_table);
 }

 function addGeneOtherAnchor(info) {
     const gene_anchor = {
         id: info.id,
         resultNum: info.geneOtherNum,
         type: 'Genes, Other',
         href: 'gene-other'
     };
     return expandTemplate('anchor', gene_anchor);
 ...
 function addGeneOtherTable() {
     const gene_table = {
         href: 'gene-other', div: 'gene-other-table', filter: false
     };
     return expandTemplate('golr-table', gene_table);
 }
