            ...
            for (var i = 0; i < info.gene_associations.length; i += 1) {
                ...
                if (ref != null) {
                    for (var j = 0; j < ref.length; j += 1) {
                        var currRef = ref[j];
                        var str = currRef.id.substring(0, 4);
                        if (str.equals("PMID") || str.equals("pmid")) {
                            var pubid = "PMID:" + currRef.id.substring(5);
                            var citation = {
                                type: "gene",
                                obj: gene,
                                pub: pubid,
        ...
        if (info.alleles != null) {
            for (var i = 0; i < info.alleles.length; i += 1) {
                ...
                var ref = info.alleles[i].evidence.split(", ");
                    if (ref != null) {
                    for (var j = 0; j < ref.length; j += 1) {
                        var str = ref[j].substring(0, 4);
                        if (str.equals("PMID") || str.equals("pmid")) {
                            var pubid = "PMID:" + ref[j].substring(5);
                            var citation = {
                                type: "allele",
                                obj: allele,
                                pub: pubid,
    ...
    try {
        ...
        if (info.genotype_associations != null) {
            for (var i = 0; i < info.genotype_associations.length; i += 1) {
                ...
                if (ref != null) {
                    var str = ref.id.substring(0, 4);
                    if (str.equals("PMID") || str.equals("pmid")) {
                        var pubid = "PMID:" + ref.id.substring(5);
                        var citation = {
                            type: "genotype",
                            obj: genotype,
                            pub: pubid,
