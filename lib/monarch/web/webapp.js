...
var fetchModelPage = function(request, id, fmt) {
    try {
        ...
        var output = pup_tent.render('model-labs.mustache', info,
...
app.get('/disease/:id.:fmt?',
        ...
        try {
            ...
            var output = pup_tent.render('disease-labs.mustache', info,
                                         'monarch_base.mustache');
            return response.html(output);
...
var fetchFeaturePage = function(request, id, fmt) {
    try {
        ...
        var ortholog_filter = { field: 'object_category', value: 'gene' };
        addGolrTable(info, "subject_closure", id, 'ortholog-table', ortholog_filter );
        ...
        // Add pathway table
        info.includes.ortholog_anchor = addOrthologAnchor(info);
        info.includes.ortholog_table = addOrthologTable();
        ...
        var output = pup_tent.render('gene-labs.mustache', info,
                 'monarch_base.mustache');
        return response.html(output);
...
function addOrthologAnchor(info) {
    var ortholog_anchor = {id: info.id, type: "Orthologs", href: "orthologs"};
    return Mustache.to_html(getTemplate('anchor'), ortholog_anchor);
}

function addOrthologTable() {
    var ortholog_table = {href: "orthologs", div: "ortholog-table"};
    return Mustache.to_html(getTemplate('golr-table'), ortholog_table);
}
...
app.get('/species/:id.:fmt?',
        function(request, id, fmt){
    
        try {
            engine.log("getting /species/:id where id="+id);
            
            //Curify ID if needed
            if (/_/.test(id)){
                engine.log("ID contains underscore, replacing with : and redirecting");
                var newID = id.replace("_",":");
                return response.redirect(genURL('species',newID));
            }

            // Species should be modeled as classes in the ontology;
            var info = {};
            info = engine.fetchClassInfo(id, {level:0}); // NOTE: level:1 takes >1m on SciGraph
            
            if (fmt != null) {
                return formattedResults(info, fmt,request);
            }
            
            addCoreRenderers(info, 'species', id);
                        
            addGolrStaticFiles(info);
            
            //Load variables for client side tables
            //We allow non-species grouping taxa, so we pin on subject_taxon_closure
            var phenotype_filter = { field: 'object_category', value: 'phenotype' };
            addGolrTable(info, "subject_taxon_closure", id, 'phenotypes-table', phenotype_filter );
            
            var gene_filter = { field: 'subject_category', value: 'gene' };
            addGolrTable(info, "subject_taxon_closure", id, 'gene-table', gene_filter );
            
            var ortholog_filter = { field: 'relation_closure_label', value: 'homologous_to' };
            addGolrTable(info, "subject_taxon_closure", id, 'ortholog-table', ortholog_filter );
            
            var model_filter = { field: 'object_category', value: 'model' };
            addGolrTable(info, "subject_taxon_closure", id, 'model-table', model_filter );
            
            var variant_filter = { field: 'subject_category', value: 'variant' };
            addGolrTable(info, "subject_taxon_closure", id, 'variant-table', variant_filter );
            
            var pathway_filter = { field: 'object_category', value: 'process' };
            addGolrTable(info, "subject_taxon_closure", id, 'pathway-table', pathway_filter );
            ...
            info.title = 'Monarch Species: '+info.label+' ('+ info.id+')';
            ...
            var output = pup_tent.render('species.mustache', info,
                                         'monarch_base.mustache');
            return response.html(output);
