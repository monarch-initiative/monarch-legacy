...
function addGolrScratchInfo(info, query_field, id, div, filter) {
...
app.get('/labs/widget-scratch',
    function(request, page){
        ...
        addGolrScratchInfo(info, "subject_closure", id, 'bs3results');
...
app.get('/labs/widget-scratch/phenotype/:id.:fmt?',
        function(request, id, fmt){
            ...
            addGolrScratchInfo(info, "object_closure", id, 'bs3results');
...
app.get('/labs/widget-scratch/disease/:id.:fmt?',
        function(request, id, fmt){
        ...
        try {
            ...
            // Rendering.
            var info = {};
            info = engine.fetchCoreDiseaseInfo(id);
            ...
            addGolrScratchInfo(info, "subject_closure", id, 'phenotypes-table', phenotype_filter );
            
            var gene_filter = { field: 'subject_category', value: 'gene' };
            addGolrScratchInfo(info, "object_closure", id, 'gene-table', gene_filter );
            
            var model_filter = { field: 'object_category', value: 'model' };
            addGolrScratchInfo(info, "subject_closure", id, 'model-table', model_filter );
            
            // Phenogrid
            addPhenogridFiles(info);
            info.pup_tent_js_libraries.push("/phenogridloader-no-species.js");
            
            //TODO break out each of these into functions
            // Add phenotype table
            var phenotype_anchor = {id: info.id, type: "Phenotypes", href: "phenotypes"}
            var phenotype_table = {href: "phenotypes", div: "phenotypes-table"}
            info.includes.phenotype_anchor = 
                Mustache.to_html(getTemplate('anchor'), phenotype_anchor);
            info.includes.phenotype_table = 
                Mustache.to_html(getTemplate('golr-table'), phenotype_table);
            
            // Add gene table
            var gene_anchor = {id: info.id, type: "Genes", href: "genes"}
            var gene_table = {href: "genes", div: "gene-table"}
            info.includes.gene_anchor = 
                Mustache.to_html(getTemplate('anchor'), gene_anchor);
            info.includes.gene_table = 
                Mustache.to_html(getTemplate('golr-table'), gene_table);
            
            // Add model table
            var model_anchor = {id: info.id, type: "Models", href: "models"}
            var model_table = {href: "models", div: "model-table"}
            info.includes.model_anchor = 
                Mustache.to_html(getTemplate('anchor'), model_anchor);
            info.includes.model_table = 
                Mustache.to_html(getTemplate('golr-table'), model_table);
            
            info.includes_phenotype_table
...
//Example: /disease/DOID_12798/phenotype_associations.json
//Currently only works for json or rdf output
app.get('/labs/widget-scratch/disease/:id/:section.:fmt?', function(request, id, section, fmt) {
    var newId = engine.resolveClassId(id);
    if (newId != id) {
        engine.log("redirecting: "+id+" ==> "+newId);
        return response.redirect(genURL('disease',newId));
...
app.get('/labs/widget-scratch/gene/:id.:fmt?',
        function(request, id, fmt){
...
app.get('/labs/widget-scratch/genotype/:id.:fmt?',
        function(request, id, fmt){
            ...
            var output = pup_tent.render('genotype-labs.mustache', info,
					 'monarch_base.mustache');
            return response.html(output);
            return res;
 });
