...
app.get('/labs/gene/:id.:fmt?', function(request, id, fmt) {
 ...
 app.get('/genotypes/:id.:fmt?', function(request, id, fmt) {

    var info;
    // if ZFIN prefix, then strip it off
    var prefix = id.indexOf("ZFIN:");
    if ( prefix > -1) {
        id = id.substr(5, id.length);
    }
    try {
        info = engine.fetchGeneAssociations(id);
    }
    catch(err) {
        return errorResponse(err);
    }
    console.log("Found info...now parsing..")
    var associations = [], found = false;
    ...
    for (var x in edges) {
        if (edges[x].obj == id) {
            console.log("obj: " + edges[x].obj + " sub:" + edges[x].sub);
            found = false;
            for (var n in nodes) {
                if (nodes[n].id == edges[x].sub) {
                    var a = {id: edges[x].sub, label: nodes[n].lbl};
                    associations.push(a);
                    found = true;
            ...
            // collect all sub that were not found
            subs.push(edges[x].sub);
    
        }
    }
 /* this gets the other edge sub, but the id*/
    console.log("checking subs..."+subs.length);
    for(var s in subs) {
        for (var x in edges) {
            if (edges[x].obj == subs[s]) {
                console.log("obj: " + edges[x].obj + " sub:" + edges[x].sub + " pred: " + edges[x].pred);
                //console.log(x);
                for (var n in nodes) {
                    if (nodes[n].id == edges[x].sub) {
                        var lbl = nodes[n].lbl;
                        var i=0, found =false;
                        while(!found && i < associations.length) {
                            var v = associations[i];
                            if (v.label == lbl) {found=true;}i++;
                        }
                        if (!found) {
                            var a = {id: edges[x].sub, label: lbl};
                            associations.push(a);
                        }
                    //console.log(a.id + " " + a.label);
                    //break;
