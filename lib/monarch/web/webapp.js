...
app.get('/disease/:id.:fmt?', function(request, id, fmt) {
    try {
        ...
        if (info.gene_associations != null) {
            for (var i = 0; i < info.gene_associations.length; i += 1) {
                var gene = info.gene_associations[i].gene;
                var ref = info.gene_associations[i].references;
                if (ref != null) {
                    for (var j = 0; j < ref.length; j += 1) {
                        ...
                        if (str.equals("PMID:") || str.equals("pmid:")) {
                            ...
                            var citation = {
                                ...
                                obj: gene,
                                pub: pubid,
                                pubinfo: engine.fetchPubFromPMID(num),
                            };
        ...
        if (info.phenotype_associations != null) {
            for (var i = 0; i < info.phenotype_associations.length; i += 1) {
                var phenotype = info.phenotype_associations[i].phenotype;
                var ref = info.phenotype_associations[i].references;
                if (ref != null) {
                    for (var j = 0; j < ref.length; j += 1) {
                        var currRef = ref[j];
                        var str = currRef.id.substring(0, 5);
        ...
        if (info.alleles != null) {
            for (var i = 0; i < info.alleles.length; i += 1) {
                var allele = info.alleles[i].allele;
                var ref = info.alleles[i].evidence.split(", ");
                    if (ref != null) {
                    for (var j = 0; j < ref.length; j += 1) {
        ...
        info.hasLiterature = function() {return checkExistence(literature)};
        ...
        info.literatureNum = function() {return getNumLabel(literature)};
        ...
        info.literatureTable = function() {return genTableOfLiterature(literature);};
...
app.get('/phenotype/:id.:fmt?', function(request, id, fmt) {
    ...
    try {
        ...
        if (info.disease_associations != null) {
            for (var i = 0; i < info.disease_associations.length; i += 1) {
                var disease = info.disease_associations[i].disease;
                var ref = info.disease_associations[i].references;
                if (ref != null) {
                    for (var j = 0; j < ref.length; j += 1) {
        ...
        if (info.genotype_associations != null) {
            for (var i = 0; i < info.genotype_associations.length; i += 1) {
                var genotype = info.genotype_associations[i].has_genotype;
                var ref = info.genotype_associations[i].reference;
                if (ref != null) {
...
var fetchGenotypePage = function(request, id, fmt) {
    try {
        ...
        if (info.phenotype_associations != null) {
            for (var i = 0; i < info.phenotype_associations.length; i += 1) {
                var phenotype = info.phenotype_associations[i].has_phenotype.type;
                var ref = info.phenotype_associations[i].reference;
                if (ref != null) {
                    var str = ref.id.substring(0, 5);
                    var num = ref.id.substring(5);
                    if (str.equals("PMID:") || str.equals("pmid:")) {
...
app.get('/gene/:id.:fmt?', function(request, id, fmt) {
    ...
    if (info.interactions != null) {
        for (var i = 0; i < info.interactions.length; i += 1) {
            var gene = info.interactions[i].geneb;
            var ref = info.interactions[i].references;
            if (ref != null) {
                var str = ref.id.substring(0, 5);
                var num = ref.id.substring(5);
                if (str.equals("PMID:") || str.equals("pmid:")) {
