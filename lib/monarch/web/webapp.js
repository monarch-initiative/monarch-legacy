...
function searchHandler(request, term) {
    ...
    try {
       ...
       // }
        ...
        //var results = engine.search(term, {}, ["category", "leaf", "taxon_label"], resultsCount, 0);
...
function caseByIdHandler(request, id, fmt) {
    try {
        ...
        addGolrTable(info, "subject_closure", id, 'phenotypes-table', phenotype_filter, 'case_phenotype', '#phenotypes');
        info.phenotypeNum = engine.fetchAssociationCount(id, 'subject_closure', phenotype_filter, 'object');


        var variant_filter = [{ field: 'object_category', value: 'variant' },
                              { field: 'subject_category', value: 'case' }];
        addGolrTable(info, "subject_closure", id, 'variant-table', variant_filter, 'case_variant','#variants');
        info.variantNum = engine.fetchAssociationCount(id, 'subject_closure', variant_filter, 'object');
...
function fetchCaseSection(request, id, section, fmt) {
...
web.wrapRouteGet(app, '/case/:id/:section.:fmt?', '/case/{id}/{section}.{fmt?}', ['id', 'section', 'fmt'], fetchCaseSection);
