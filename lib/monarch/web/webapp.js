...
/* Here we are overriding the function upload from the upload.js module in stick
 ...
 * tmp file.  We also set a limit of 50 mb by checking the content-length in
 * HTTP Header, but this should be checked elsewhere since this can be manipulated
 * overriding parseFileUpload may be the only option here
 */

// Custom upload function to upload files to tmp file
// Set upload limit to <50 MB using content-length from HTTP Header
var {isFileUpload, parseFileUpload, TempFileFactory} = require("ringo/utils/http");
            ...
            get: function() {
                if (!postParams) {
                    ...
                    if (req.headers['content-length'] > 50000000){
                        ...
                        parseFileUpload(this, postParams, encoding, TempFileFactory);
                    } else if (desc) {
...
app.get('/neighborhood/:id/:depth/:direction/:relationship.:fmt?', function(request, id, depth, direction, relationship, fmt) {
    var info = engine.getGraphNeighborsExtend(id, depth, relationship, direction);
...
//    var info = engine.fetchAttributeComparisonMatrix(x,y);
    ...
    var info = engine.compareEntities(x,ys);
...
app.get('/dynamic*.:fmt?', function(request) {
...
app.get('/dynamic/gene/:id/genotype/nodes.:fmt?', function(request,id,fmt) {

	//this will end up going to scigraph in the future; for now it wraps the
    //nif call
    //var info = engine.fetchGeneInfo(id);
    //var geno_assoc = info['genotype_associations'];

	//convert the association objects into the node array of genotypes
	//if (geno_assoc != null) {
	//	genotypes = 
	//		geno_assoc.map( 
	//			function(g) {
	...
	//var res = {'nodes' : genotypes};

	//we can also wrap the genotypes from gene call:
    //for testing:MGI:98297
    
    var params = {'gene_id' : id};
	var path = '/dynamic/genotypes_from_gene.json';
	
	var scigraph_results = engine.querySciGraphDynamicServices(path,params);

	var res = null;
	if (scigraph_results != null) {
		res = { 'nodes' : scigraph_results['nodes'] };
	}
	console.log("fmt="+fmt);
	if (fmt == 'json') {
		return response.json(res);
	} else {
		return response.error('unsupported format.  please use .json');
	}
...
app.get('/labs/gene/:id.:fmt?', function(request, id, fmt) {
 ...
 app.post('/labs/analyze/:datatype.:fmt?', function(request, datatype, fmt) {
     ...
     if (typeof request.params.file_exceeds != 'undefined'){
         ...
         if (fs.size(fileUpload.tempfile) > 50000000){ //Another sanity check
     ...
     var output = pup_tent.render('analyze-labs.mustache',info,'monarch_base.mustache');
     return response.html(output);
 });
 
 app.get('/labs/analyze/:datatype.:fmt?', function(request, datatype, fmt) {
     var target = request.params.target;
     var species = request.params.target_species;
