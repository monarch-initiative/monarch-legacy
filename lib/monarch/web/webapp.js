        ...
        if (info.results.response.docs.length > 0) {
            ...
            for (var i = 0; i < docs.length; i++) {
                ...
                if (typeof(docs[i].taxon_label) !== 'undefined') {
                    ...
                    // why won't work?
	                var filteredCategory = docs[i].category.filter(function(cat) {
                        if (cat.indexOf(' ') != -1) {
                            return cat.split(' ').join('_').toLowerCase();
                        } else {
...
console.log(filteredCategory);
	                ...
	                info.resultsOutput += '<tr class="search-result-item ' + taxonSelector + ' ' + filteredCategory.join(" ") + '">'
