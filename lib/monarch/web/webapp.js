...
app.get('/disease/:id.:fmt?', function(request, id, fmt) {
    try {
        ...
        info.annotationScore = function() {
            if (info.phenotype_associations.annotation_sufficiency != null) {
                return (5 * info.phenotype_associations.annotation_sufficiency);
            } else {
                return 0;
			}
        };
        ...
        info.geneNum = function() {
			if (info.gene_associations != null) {
				return getNum(engine.unique(info.gene_associations.map(function(g) {return g.gene.id})))};
			return 0;
			};
        ...
        info.pathwayNum = function() {return getNum(info.pathway_associations)};
...
app.get('/gene/:id.:fmt?', function(request, id, fmt) {
    ...
    info.diseaseNum     = function() {return getNum(info.disease_associations)};
    info.alleleNum      = function() {return getNum(info.alleles)};
    info.orthologNum    = function() {return getNum(info.orthologs)};
    info.interactionNum = function() {return getNum(info.interactions)};
	...
	if (info.tax_xref){
		info.taxon_xref = function() {return genExternalHref('source',info.tax_xref)};
	}
