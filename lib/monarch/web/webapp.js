...
function getConfigHandler(_request) {
  // console.log('getConfigHandler app', app);
  // console.log('getConfigHandler saveThis', saveThis);
  // console.log('getConfigHandler saveThis.defaultConfig', saveThis.defaultConfig);
  // console.log('getConfigHandler saveThis.golrConfig', saveThis.golrConfig);
  const serverConfigurationName = 'development';
  const configAsJS = JSON.stringify(app.defaultConfig, null, 2);
  const configGOLRAsJS = JSON.stringify(app.golrConfig, null, 2);
  const configScript =
`
window.serverConfigurationName = '${serverConfigurationName}';

window.serverConfiguration = ${configAsJS};

window.serverConfigurationGOLR = ${configGOLRAsJS};

// console.log('window.serverConfigurationName', window.serverConfigurationName);
// console.log('window.serverConfiguration', window.serverConfiguration);
// console.log('window.serverConfigurationGOLR', window.serverConfigurationGOLR);
`;

  // console.log('getConfigHandler', serverConfigurationName);
  // console.log(configScript);
  return web.wrapJS(configScript);
}

web.wrapRouteGet(app, '/configuration/server.js', [], getConfigHandler, errorResponse);
...
if (useSPA) {
  ...
  function nodeByTypeIdHandler(request, nodetype, id) {
      ...
      // Rendering.
      var info = newInfo(engine.fetchClassInfo(cliqueLeader, {level:1}));
...
app.configServer = function(defaultConfig, golrConfig) {
    ...
    app.golrConfig = golrConfig;
};

app.startServer = function() {
    engine = buildEngine(app.defaultConfig, app.golrConfig);
    var that = this;
