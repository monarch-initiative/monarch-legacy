...
function _get_blog_data(){
	...
	try {
		...
		// For apparent inability for this XML lib to deal
		// with colons.
		c = c.replace(/atom\:updated/g, "modDate");
		var rss = new XML(c);
		if( rss && rss.channel && rss.channel.item ){
			...
			// Pubdate.
			var pt = new Date(item['pubDate']);
			var py = pt.getFullYear();
			var pm = pt.getMonth();
			if( pm < 10 ){ pm = '0' + pm; }
			var pd = pt.getDate();
			
			// Moddate. Ringo cannot parse dates properly,
			// so we'll have to do a little of this by
			// hand.
			var mdate = item['modDate'];
			var mda = mdate.split('T');
