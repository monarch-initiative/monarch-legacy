var {Application} = require('stick');
var Mustache = require('ringo/mustache');

var e = new bbop.monarch.Engine();

e.cache = {
    var fs = require('fs');
    store: fetch(tbl, key, val) {
        string path = "./cache/"+tbl+"/key-"+key+".cached";
        if (fs.exists(path)) {
            return JSON.parse(fs.read('templates/'+t+'.mustache'));
        }
        return null;
    }
    store: function(tbl, key, val) {
        // TODO
    }
};

var app = exports.app = new Application();
app.configure('route');

function getTemplate(t) {
    var fs = require('fs');
    var s = fs.read('templates/'+t+'.mustache');
    return s;
}

app.get('/test/:a/:b', function(request, a, b) {
   return {
      body: [Mustache.to_html(getTemplate('test'), {a:a, b:b})],
      headers: {'Content-Type': 'text/html'},
      status: 200
   }
});

app.get('/hello', function(request) {
   return {
      body: ['<h1>Hello world</h1>'],
      headers: {'Content-Type': 'text/html'},
      status: 200
   }
});

app.get('/disease/:id/json', function(request, id) {
    var info = e.fetchDiseaseInfo(id); 
    var payload = JSON.stringify(info);
    
   return {
      body: ['Hello '+payload],
      headers: {'Content-Type': 'text/html'},
      status: 200
   }
});

// Example: http://127.0.0.1:8080/disease/DOID_14330
app.get('/disease/:id', function(request, id) {
    var info = e.fetchDiseaseInfo(id); 

    var html = diseasePage(info) + "";
    
    return {
      body: [html],
      headers: {'Content-Type': 'text/html'},
      status: 200
   }
});

app.get('/disease2/:id', function(request, id) {
    var info = e.fetchDiseaseInfo(id); 
    //var info = {label: "foo"};
    
    info.tableOfPhenotypes = function() {return genTableOfPhenotypes(info)} ;

    return {
      body: [Mustache.to_html(getTemplate('disease'), info) ],
      headers: {'Content-Type': 'text/html'},
      status: 200
   }
});

if (require.main == module) {
   require('ringo/httpserver').main(module.id);
}
