...
=======
        //var results = engine.search(term, null, ["category", "taxon"], resultsCount, 1);
>>>>>>> b21575f28eb7ed88bda42cd586ad30dc3c6f9faa
...
/*        info.results.forEach( function (i) {
        ...
        if (info.results.response.docs.length > 0) {
            ...
            for (var j = 0; j < taxons.length; j++) {
                ...
                if (taxons[j][0] === '') {
                    ...
                    info.resultsSpeciesFilter += '<button class="search-results-button" data-filter=".' + taxonClassSelector + '">' + taxons[j][0] + '</button>';
                }
            ...
            for (var k = 0; k < categories.length; k++) {
                info.resultsCategoryFilter += '<button class="search-results-button" data-filter=".' + categories[k][0] + '">' + categories[k][0] + '</button>';
            }
            ...
            for (var i = 0; i < docs.length; i++) {
                ...
                if (typeof(docs[i].taxon_label) !== 'undefined') {
	                ...
	                var filteredCategory = docs[i].category;
	                // filteredCategory.filter(function(cat){
	                //     return cat = cat.split(' ').join('_').toLowerCase();
	                // });

	                // Don't forget the space to sprarate multi CSS classes
	                info.resultsOutput += '<div class="search-result-box ' + taxonSelector + ' ' + filteredCategory.join(" ") + '">'
	                    + '<ul>'
	                    + '<li>Term: ' + genObjectHref(docs[i].category, docs[i]) +'</li>'
	                    + '<li>Category: ' + docs[i].category.join(", ") + '</li>'
	                    + '<li>Taxon: ' + docs[i].taxon_label + '</li>'
	                    + '</ul>'
	                    + '</div>';
                }    
            }
        ...
        // isotope
        info.pup_tent_js_libraries.push("/isotope.pkgd.min.js");
