...
var pup_tent = require('pup-tent')(['js','css','templates','templates/labs',
                                    ...
                                    'widgets/modeltype/js',
                                    'widgets/modeltype/css',
...
app.get('/widgets/modeltype/js/:page', function(request,page) {
    return ('widgets/modeltype/js',page,'text/html');
});

app.get('/widgets/modeltype/js/:page', function(request,page) {
    return ('widgets/modeltype/js',page,'text/plain');
});*/
...
app.get('/search/:term?.:fmt?', function(request, term, fmt) {
    try {
        ...
        info.pup_tent_css_libraries.push("/search.css");
...
app.get('/disease/:id.:fmt?', function(request, id, fmt) {
    try {
        ...
        info.pup_tent_css_libraries.push("/modelviewer.css");
        
        info.pup_tent_js_libraries.push("/keggerator.js");
        info.pup_tent_js_libraries.push("/modeltype.js");
...
app.get('/labs/datagraph.:fmt?',function(request,fmt){
        ...
        var info = engine.generateHistogram("HP:0000118",6,'true');
	    ...
	    info.phenoGraph = function() {return genPhenoGraph(info.dataGraph)};
...
app.get('/labs/cy-path-demo',
 ...
 //app.static(module.resolve("../../../labs/jbrowse"), "index.html"); // serve files in docs as static resources
 //app.static(module.resolve("../../../labs/jbrowse"), "templates/labs/jbrowse/index.html"); // serve files in docs as static resources
 //app.static(module.resolve("templates/labs/jbrowse")); // serve files in docs as static resources
 //app.static(module.resolve("jbrowse")); // serve files in docs as static resources
 //app.static(module.resolve("jbrowse"),"index.html"); // serve files in docs as static resources
 //app.static("/labs/jbrowse/"
 //    //express.static(__dirname + "/jbrowse")
 //);


 app.get('/labs/jbrowse-demo',
     function(request, page){
         var info = {};
         addCoreRenderers(info);
         ...
         info.title = 'JBrowse Start';

         //return response.html(Mustache.to_html(getTemplate('labs/cy-path-demo'), info));
         var output = pup_tent.render('jbrowse.mustache',info,'monarch_base.mustache');
         return response.html(output);
     });


 //app.get('/labs/jbrowse/jbrowse/*',
 //    function(request, page){
 //        // TODO: add pass-through to file-system
 //        var info = {};
 //        addCoreRenderers(info);
 //
 //        // Now add the stuff that we need to move forward.
