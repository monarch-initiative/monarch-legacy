...
// Comment out since we are not using table to render search results - Zhou
...
function searchHandler(request, term) {
    ...
    try {
       ...
       // }
        ...
        // Only display 50 results for now
        var resultsCount = 50;
...
/*        info.results.forEach( function (i) {
        ...
        if (info.results.response.docs.length > 0) {
            ...
            info.count = info.results.response.numFound;

            info.hasResults = true;
            
            info.resultsCount = 'Total number of results: ' + info.results.response.numFound;
            ...
            // We need to exclude "Class" and "sequence feature" from the list
            categories = categories.filter(function(item){
                return (item[0] === 'Class' || item[0] === 'sequence feature') ? false : true;
            });
            ...
            for (var j = 0; j < taxons.length; j++) {
                ...
                if (taxons[j][0] === '') {
                    info.resultsSpeciesFilter += '<button class="search-results-button" data-filter=".not_specified">Not Specified</button>';
                } else {
                    ...
                    info.resultsSpeciesFilter += '<button class="search-results-button" data-filter=".' + taxonClassSelector + '">' + taxons[j][0] + '(' + taxons[j][1] + ')</button>';
                }
            ...
            info.resultsCategoryFilter = '<div class="search-results-button-group js-radio-button-group" data-filter-group="category">'
                + '<button class="search-results-button is-checked" data-filter="">All Categories</button>';
            for (var k = 0; k < categories.length; k++) {
                // Must have dot . to prefix the class in data-filter
                info.resultsCategoryFilter += '<button class="search-results-button" data-filter=".' + categories[k][0].toLowerCase() + '">' + categories[k][0].toLowerCase() + '(' + categories[k][1] + ')</button>';
            }
            info.resultsCategoryFilter += '</div>';

            // Create the new output format - Zhou
            info.resultsOutput = '<table class="search-results-table table table-condensed simpletable">'
                + '<thead><tr><th width="20%">Term</th><th width="20%">Category</th><th>Taxon</th></tr></thead>'
                + '<tbody>';
            ...
            for (var i = 0; i < docs.length; i++) {
                ...
                if (typeof(docs[i].taxon_label) !== 'undefined') {
                    // convert space separateed taxon name into underscore separated CSS class - Zhou
	                if (docs[i].taxon_label === '') {
	                    var taxonSelector = 'not_specified';
	                } else {
	                    var taxonSelector = docs[i].taxon_label.split(' ').join('_').toLowerCase();
	                }

                    // Only use category[1], exclude Class, and sequence feature - Zhou
	                // Don't forget the space to sprarate multi CSS classes
	                info.resultsOutput += '<tr class="search-result-item ' + taxonSelector + ' ' + docs[i].category[1] + '">'
	                    + '<td>' + genObjectHref(docs[i].category, docs[i]) +'</td>'
	                    + '<td>' + docs[i].category[1] + '</td>'
	                    + '<td>' + docs[i].taxon_label + '</td>'
	                    + '</tr>';
                }    
            }
