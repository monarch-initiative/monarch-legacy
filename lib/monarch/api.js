    ...
    if (resultObj.results.length > 0){
        ...
        if (gene.references[0]){
            ...
            for (var i=0;i < gene.references.length; i++){
            	...
            	var modelObj = this.fetchModelOrgGenePhenotypeAsAssociation(gene.references[i],gene);
            	var genoTypeObj = this.fetchGeneGenoTypeAsAssociation(gene.references[i],gene);
...
bbop.monarch.Engine.prototype.fetchGeneGenoTypeAsAssociation = function(id,genObj) {
    ...
    if ((id.source == 'MGI')||(id.source == 'ZFIN')||(id.source == 'WB')){
        var resource_id = source_to_resource_map[id.source].id;
        var filter;
        if ((id.source == 'MGI')||(id.source == 'ZFIN')){
        	filter = ["implicated_gene_ids:"+this.quote(id.id)];
        } else if (id.source == 'WB'){
            filter = ["gene_id:"+this.quote(id.id)];
        }
        var tr;
        ...
        if ((id.source == 'MGI')||(id.source == 'ZFIN')){
            ...
            var obj = {
                ...
                genotype : { 
                 ...
                 source : {
                     id : resource_id, 
                     label : source_to_resource_map[id.source].label
                 },
...
bbop.monarch.Engine.prototype.fetchModelOrgGenePhenotypeAsAssociation = function(id,genObj) {
