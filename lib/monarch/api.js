...
bbop.monarch.Engine = function(opts) {
    ...
    this.config.owlsim_services_url = "http://toaster.lbl.gov:9031";
...
bbop.monarch.Engine.prototype.fetchGenotypeInfo = function(id, opts) {
    ...
    // TODO - do not assume all genotypes are from MGI. <-- add corresponding ZFIN call
    ...
    // fix this when we can get genotype info directly from view
    if (obj.label == null) {
        console.log("Temporary measure: getting genotype info from geno-pheno associations.");
        var proto = obj.phenotype_associations[0].has_genotype;
        // copy
        for (var k in proto) {
            obj[k] = proto[k];
        }
        // obj.has_bcackground = proto.has_genotype;
        // obj.has_part = proto.has_part;
        // obj.label = proto.label;
        // obj.type = proto.type;
    }

    // TEMPORARY - until we query genotype table directly for ZFIN
    // if (obj.id == null) {
    //     var genotype = obj.phenotype_associations[0].has_genotype;
    //     obj.id = genotype.type.id;
    //     obj.label = genotype.type.label;
    // }
...
bbop.monarch.Engine.prototype.fetchMgiGenoPhenoAsAssocations = function(id) {
    ...
    var tr =
        function (r) {
            ...
            var background = {
                id : r.background_strain_id,
                label: r.background_strain_name ,
                type : "genomic_background"
            };

            // organism part of genotype: broken into background and genomic variation complement
            var genotype_context =
                {
                    has_part : [
                        background
                        //gv_complement <-- todo - check if we have this normalized out
                    ],
                    type : "genotype_context"
                };
            ...
            var obj = {
                ...
                has_genotype : 
                {
                    ...
                    has_part : [
                        // experimental_context, <-- TODO, check - don't think we have this
                        genotype_context
                    ]
                ...
                source : {
                    id : resource_id,
                    label : "MGI GenoPheno associations",
                },                    
                resource : resource_id
            };
            return obj;
...
bbop.monarch.Engine.prototype.fetchZfinGenoPhenoAsAssocations = function(id) {
    ...
    var tr =
        function (r) {
            ...
            // experimental part of genotype
            var experimental_context = 
                {
                    id : r.extrinsic_genotype_id,
                    label : r.extrinsic_genotype_label_html,
                    type : "experimental_context"
                };

            var background =
                { 
                    id : r.background_strain_id,
                    label : r.background_strain_label,
                    type : "genomic_background"
                };

            //gvc not presently supplied in this view
            var gv_complement =
                {
                    id : r.intrinsic_genotype_id,
                    label : r.intrinsic_genotype_label_html,
...
//                        gv_complement
            ...
            var obj = {
                id : r.zfin_annotation_id, // TODO - push ID generation to ingest
                evidence : { 
                    type : {
                        id : "ECO:0000006",
                        code : "EXP",
                ...
                has_genotype : 
                {
                    ...
                    label : r.effective_genotype_label_html,
                    type : "genetic_context",
                ...
                source : {
                    id : resource_id,
                    label : "ZFIN GenoPheno associations",
                },                    
                resource : resource_id
            };
            return obj;
 ...
 */
bbop.monarch.Engine.prototype.fetchDataFromResource = function(id, resource_id, trFunction, projectCols) {
    // Example: http://beta.neuinfo.org/services/v1/federation/data/nif-0000-03216-7.json?includePrimaryData=true&q=Smith
    var nif_id = this.getNifId(id);

    var LIMIT = 1000; // System does not allow >1k results at a time
    var params = 
        {
...
bbop.monarch.Engine.prototype.getJsonLdContext = function() {
    var ctxt =
    {
        ...
        genomic_background : "GENO:0000032",
