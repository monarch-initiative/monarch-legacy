...
bbop.monarch.Engine.prototype.fetchClassInfo = function(id, opts) {
	...
	var tr = function (r) {
        ...
        var obj = {
            'id' : r.list[0].curie,
            'label' : r.list[0].labels[0],
            'url' : r.list[0].uri,
            'category' : r.list[0].categories[0],
            'has_exact_synonym' : r.list[0].synonyms,
            'database_cross_reference' : []
	    }; 
    	return obj;
    };
    
	resultObj = this.getVocabularyByID(id,tr);
...
// TODO This could cover both /search/term and /term/term
bbop.monarch.Engine.prototype.getVocabularyByTerm = function(term,limit,searchSynonyms,category,ontology,trFunction) {
	...
	var path = "vocabulary/search/"+term+'.json';
	var uri = this.config.scigraph_url+path;
	var sciObj;
    var resultObj = [];
    var resultStr;
...
bbop.monarch.Engine.prototype.querySciGraph = function(uri,params,trFunction) {
    ...
    if (trFunction){
    	if (sciObj instanceof Array){
    		...
    		//TODO figure out how to approach objects
    		/*sciObj[list].forEach(function(r) {
                resultObj.push( trFunction(r) );
            });*/
    		resultObj = {};
    		var result = trFunction(sciObj);
    		if (result){
                resultObj = result;
    		}
    	}
