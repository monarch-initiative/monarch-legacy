 ...
 *            - category: A category, available categories include:
 ...
 * 
 */
bbop.monarch.Engine.prototype.fetchAssociations = function(id, field, category, limit) {
    var engine = this;
    ...
    golrManager.set_personality('generic_association');
    golrManager.add_query_filter('object_closure', id, ['*']);
    golrManager.set_results_count(limit);
    
    if (typeof category === 'undefined'){
        golrManager.add_query_filter('subject_closure_category', category, ['*']);
    }
...
bbop.monarch.Engine.prototype.searchSubstring = function(term,categories,limit) {
    ...
    results.forEach(function(r,i,resultList){
        ...
        if (/^OBO:NCBIGene_/.test(resultList[i].id)){
            resultList[i].id = 
                resultList[i].id.replace(/^OBO:NCBIGene_/, "NCBIGene:");
            resultList[i].curie = resultList[i].id;
        }
    }); 
