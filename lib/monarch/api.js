...
bbop.monarch.Engine.prototype.fetchOrthologList = function(ids) {
    ...
    var filter = "^MGI|ZFIN|ENSEMBL:ENSG\\d+|ENSEMBL:ENSDARG\\d+|ENSEMBL\:ENSMUSG\\d+";
...
bbop.monarch.Engine.prototype.fetchGeneOrthologFromSciGraph = function(id) {
    ...
    var CLASS_MAP = {
            'RO_HOM0000017' : 'Ortholog',
            'RO_HOM0000020' : 'Least Diverged Ortholog',
            'RO_HOM0000019': 'Homolog',
            'RO_HOM0000011' : 'Paralog',
            'RO_HOM0000023' : 'Paralog',
            'RO_HOM0000022' : 'Ohnolog',
            'RO_HOM0000018': 'Xenolog',
            'RO_0002351' : 'has_member'       
    };
    
    var params = {
            'homolog_id': 'RO_HOM0000000'
    };
...
bbop.monarch.Engine.prototype.compareEntities = function(x,y) {
	...
	//expand each item in the query to it's phenotype, and make a union list
	var xphenotype_ids = engine.getPhenotypes(x);
    //for (var it=0; it<x.length; it++) {
	//	ids = this.getPhenotypes(x[it]);
	//	if (ids.map != null) {
    ...
    for (var it=0; it<y.length; it++) { 
		...
		var n = engine.getGraphNodeByID(target_id);
        ...
        stuff.taxon = engine.mapMatchIdentifierToTaxon(target_id);
	...
	var a = {};
	if (x.length == 1) {
		var n = engine.getGraphNodeByID(x[0]);
		var graph = new bbop.model.graph();
		graph.load_json(n);
		...
		a.taxon = engine.mapMatchIdentifierToTaxon(x[0]);  //TODO replace with a scigraph call
    } else {
        var xlabels = [];
		x.forEach( function (xid) {
			var n = engine.getGraphNodeByID(xid);
			var graph = new bbop.model.graph();
			graph.load_json(n);
...
bbop.monarch.Engine.prototype.getPhenotypes = function(id_list) {
    ...
    for (var it=0; it<id_list.length; it++) {
		var id = id_list[it];
	    if (this.mapIdentifierType(id) == 'phenotype') {
			//TODO get the phenotype labels?
		    expanded_list.push(id);
		} else if (this.mapIdentifierType(id) == 'disease') {
			this.log("Expanding disease ID to phenotype: "+id);
			//call scigraph services to get the disease phenotypes
			var nodes = engine.fetchPhenotypes(id,'diseases');
			expanded_list = expanded_list.concat(nodes.map( function(i) {return i.id}) );
		} else if (this.mapIdentifierType(id) == 'gene') {
			this.log("Expanding gene ID to phenotype: "+id);
			var nodes = engine.fetchPhenotypes(id,'genes');
			expanded_list = expanded_list.concat(nodes.map( function(i) {return i.id}) );

		} else if (this.mapIdentifierType(id) == 'genotype') {
			this.log("Expanding genotype ID to phenotype: "+id);
			this.log("Expanding genotype to phenotype not yet implemented.");		
		}
			//figure out what type of thing it is being queried
			//TODO use services to figure out what this thing is
			//var types = engine.getNodeTypes(id);
	}
...
bbop.monarch.Engine.prototype.fetchPhenotypes = function(id,pathtype) {
//queryies SciGraph dynamic services to get the phenotype nodes.
//This does not get the paths, just the terminal nodes.
//For example: http://rosie.crbs.ucsd.edu:9000/scigraph/dynamic/diseases/OMIM:234500/phenotypes/targets
...
bbop.monarch.Engine.prototype.fetchClassInfo = function(id, opts) {
    ...
    var res = engine.getVocabularyTermByID(id);//rename get vocabulary term by id
...
bbop.monarch.Engine.prototype.searchOverOntologies = function(term, opts) {
    ...
    results.forEach(function(r,i,concepts){
        ...
        //HACK - REFACTOR OUT WHEN GENE CURIES ARE FIXED
        if (/^OBO:NCBIGene_/.test(concepts[i].id)){
            concepts[i].id = 
                concepts[i].id.replace(/^OBO:NCBIGene_/, "NCBIGene:");
        }
...
bbop.monarch.Engine.prototype.getEquivalentClassList = function(id,level) {
    ...
    var neighbors = engine.getGraphNeighbors(id, level, relationshipType);
...
//
bbop.monarch.Engine.prototype.getGraphNeighbors = function(id, depth, relationshipType, direction) {
    ...
    var resultObj = [];
    resultObj = this.querySciGraphOntologyServices(path,params);
    return resultObj;
}
...
bbop.monarch.Engine.prototype.getGraphNodeByID = function(id) {
    var engine = this;
    var params = null; //No parameters for this operation
...
bbop.monarch.Engine.prototype.getSciGraphXRefs = function(id) {
    ...
    resultObj = engine.querySciGraphOntologyServices(path);
...
bbop.monarch.Engine.prototype.getNodeTypes = function(id) {
    ...
    var resultObj = engine.querySciGraphOntologyServices(path,params);
	var types = [];
...
bbop.monarch.Engine.prototype.querySciGraphOntologyServices = function(path,params) {
 ...
 * 
 */
bbop.monarch.Engine.prototype.fetchAssociations = function(id, field, filter, limit) {
    var engine = this;
    
    if (typeof limit === 'undefined'){
        limit = 50000; //Set to some high number
    }
    ...
    golrManager.set_personality('generic_association'); //hardcode for now
    ...
    var raw = engine.fetchUrl(golrManager.search());
...
bbop.monarch.Engine.prototype.fetchAssociationCount = function(id, field, filter) {
    var golrResponse = engine.fetchAssociations(id, field, filter, 0);
    return golrResponse.total_documents();
}
 ...
 * Status: Partially Implemented
 * 
 * Fetch core information related to a gene, such as definition, xrefs
 * For now also get parent and subclass(es) and phenotype associations
 * 
 * Resources: SciGraph and GOlr
 * 
 ...
 * 
 */
bbop.monarch.Engine.prototype.fetchCoreGeneInfo = function(id) {
    var engine = this;

    // every disease is represented as a class in the ontology
    var obj = this.fetchDataInfo(id);
...
bbop.monarch.Engine.prototype.fetchDataInfo = function(id, opts) {
    ...
    var resultObj = {};
    var engine = this;
    var res = engine.getVocabularyTermByID(id, engine.config.scigraph_data_url);//rename get vocabulary term by id
    //TODO do we need to handle lists of >1
    resultObj = {};
    if (typeof res.concepts !== 'undefined' && 
    typeof res.concepts[0] != 'undefined'){
