...
bbop.monarch.Engine.prototype.fetchGeneOrthologFromSciGraph = function(id) {
    ...
    var node_list = orthologGraph.get_equivalent_classes(id);
    ...
    orthologGraph.all_edges().forEach(function(edge){
        if (equivalent_classes.indexOf(edge._subject_id) >= 0){
            var resultObj = {
                    'gene': {
                        'id': id,
                        'label': orthologGraph.get_node(id)._label
                    },
                    'ortholog' : { 
                        id : edge._object_id
                    },
...
bbop.model.graph.prototype.get_subjects = function(obj_id, pred){   
    ...
    var node_list = anchor.all_nodes();

    node_list.forEach(function(node){
    	var sub_id = node.id();
        if (anchor._sop_table[sub_id]
            && anchor._sop_table[sub_id][obj_id]
            && anchor._sop_table[sub_id][obj_id][pred]){
...
bbop.model.graph.prototype.get_equivalent_classes = function(obj_id){   
    var anchor = this;
    var retlist = new Array();
    var node_list = anchor.all_nodes();
    ...
    node_list.forEach(function(node){
        ...
        if (anchor._sop_table[sub_id]
            && anchor._sop_table[sub_id][obj_id]
            && anchor._sop_table[sub_id][obj_id]['equivalentClass']){
            var equiv_list = 
            	anchor.get_equivalent_classes(sub_id);
            var tmp_node = anchor._nodes.hash[sub_id];
            var cloned_node = tmp_node.clone();
            retlist.push(cloned_node);
