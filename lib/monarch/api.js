...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
    ...
    if (engine.cache != null) {
        var cached = engine.cache.fetch('gene', id);
        if (cached != null) {
    ...
    if (obj.source == "NOT FOUND") {
        ...
        engine.log(JSON.stringify(mappings));
	...
	gene.species = gene.taxon.id;   
   
 
    engine.log("GENE: "+JSON.stringify(gene));
	...
	//add gene citations directly from gene2pubmed list
	var refs = engine.fetchRefsForGene(id);
	literature = literature.concat(refs);
	engine.log("REFS"+JSON.stringify(literature));
...
/*
  ...
  */
	uniqResults.results = all_results;      
	return uniqResults;
}
