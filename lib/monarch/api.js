            ...
            function (r) {
                ...
                if (resource.label == 'ClinVar') {
					...
					if (r.omim_ids.split(",").length == 1 && r.omim_ids.split(",") != '') {
						...
						// but i only want to do those that map 1:1 with
...
bbop.monarch.Engine.prototype.annotateText = function(txt, opts) {
    ...
    var res = 
        this.fetchUrl("http://neuinfo.org/servicesv1/v1/annotate/complete.json", q); // TODO
																						// -
																						// config
 ...
 */
bbop.monarch.Engine.prototype.uniquifyResultSet = function(resultObj,uniqOnKeys,isSourceNew) {
	var uniqResults = [];
    var unique = {};
    uniqResults.results = [];
    	...
    	for (var i=0; i<resultObj.length; i++){
    		...
    		if (uniqOnKeys){
    			for (var j=0; j<uniqOnKeys.length; j++){
    				...
    				if (resultObj[i][uniqOnKey].id.match(/^omim/)){
