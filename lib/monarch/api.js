...
bbop.monarch.Engine.prototype.getAnnotationSufficiencyScoreFromAssociations = function(assocs) {
	...
	if (typeof assocs == 'undefined' || assocs == null || assocs == {}) {
		return 0 ;
	}
...
bbop.monarch.Engine.prototype.fetchGenotypeInfo = function(id, opts) {
	...
	obj.phenotype_associations = engine.fetchGenoPhenoAsAssociationsBySpecies(id,tax.id.toString().replace(/NCBITaxon:/,'')).results;
	...
	var pubs = engine.fetchPubsForAssociations(obj.phenotype_associations,'phenotype');

	obj.literature = pubs.literature;
	obj.pmidinfo = pubs.pmidinfo;
...
//first attempt at refactoring pub fetching
bbop.monarch.Engine.prototype.fetchPubsForAssociations = function(associations,assoc_type) {
    var literature = [];
    var pmids = [];
    var pmidinfo = [];

        for (var i = 0; i < associations.length; i += 1) {
            var source = associations[i].source;
            var ref = associations[i].reference;
            if (ref != null) {
                var regex = /^(PMID|pmid):(\d+)$/;
                var regres = regex.exec(ref.id);
                if (regres != null) {
   ...
   console.log("ROW:"+JSON.stringify(r))
    ...
    var obj =
    {
        ...
        has_part : [
         ...
         has_variant_loci : variant_loci,
        has_affected_genes : affected_gene_collection.has_part,
//[{id:r.gene_id,label:r.gene_label}],
        has_sequence_alterations : [sequence_alteration],
        taxon : { id : r.taxon_id, label : r.taxon_label}
    };
...
bbop.monarch.Engine.prototype.fetchGenoPhenoAsAssociationsBySpecies = function(id,sp) {
	...
	console.log("Fetching GenoPhenoAssociations in "+sp+" for "+id);
    var resource_id = species_to_resource_map[sp].id;
