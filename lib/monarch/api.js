...
//		'7995' : {id:'',label'zebrafish models'}
    ...
    var tr =
        function (r) {
			// these are properties of the container
            if (queryLookup[engine.getFederationNifId(r.a)]) {
                query = queryLookup[r.a];
            }
            else {
				//var id = engine.getFederationNifId(r.a);
                queryLookup[id] = {
					id : id,
...
bbop.monarch.Engine.prototype.mapGeneToNCBIgene = function(id) {
    ...
    var resultObj = this.fetchDataFromResource(null, resource_id, tr,null,["source:NCBI_gene","mapped_id:"+this.getFederationNifId(id)],null,null,{ useClosure : false});
	...
	if (resultObj.results == 0 ) {
		...
		var newid = idsplit[2];
		if (idsplit != null) {
			var resultObj = this.fetchDataFromResource(null, resource_id, tr,null,["source:NCBI_gene","mapped_id:"+this.getFederationNifId(newid),"mapped_source:"+idspace],null,null,{ useClosure : false});	
		}
	}
