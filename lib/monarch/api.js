...
bbop.monarch.Engine.prototype.fetchOrthologList = function(ids) {
    ...
    results.paralogs = results.paralogs.filter(function(d){
        return (!(/^WormBase|FlyBase/).test(d));
    });
...
bbop.monarch.Engine.prototype.fetchGeneOrthologFromSciGraph = function(id) {
    ...
    var node_list = orthologGraph.get_ancestors(id,'equivalentClass').all_nodes();
 ...
 */
bbop.model.graph.prototype.get_subjects = function(obj_id, pred){   
    var anchor = this;
    var retlist = new Array();
    
    if (anchor._os_table[obj_id]){
        Object.keys(anchor._os_table[obj_id]).forEach(function(sub_id){
            if (anchor._sop_table[sub_id][obj_id][pred]){
                var tmp_node = anchor._nodes.hash[sub_id];
                var cloned_node = tmp_node.clone();
                retlist.push(cloned_node);
            }
        });
    }
...
bbop.model.graph.prototype.get_ancestors = function(obj_id, pred){   
    var anchor = this;
    var edge_list = new Array();
    var ancestor_graph = new bbop.model.graph();
    
    anchor.get_subjects(obj_id, pred).forEach(function(sub_node){
        var sub_id = sub_node.id();
        ancestor_graph.add_edge(anchor.get_edge(sub_id, obj_id, pred));
        ancestor_graph.add_node(anchor.get_node(sub_id));
        ancestor_graph.add_node(anchor.get_node(obj_id));
        ancestor_graph.merge_in(anchor.get_ancestors(sub_id, pred));
    });
