...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
	...
	if ( typeof gene.taxon != 'undefined' && gene.taxon.id == 'NCBITaxon:9606') {
...
//this will take the results of either a search
//or a comparison (by attribute set), and make the same return object
...
bbop.monarch.Engine.prototype.compareEntities = function(x,y) {
	...
	var stuff = {};
	stuff.matches = engine.fetchAttributeComparisonMatrix(x,y);
	stuff.j = { id : y, label : engine.getLabel(y), type : engine.mapIdentifierType(y), id_list : yphenotype_ids};
	stuff.taxon = engine.mapMatchIdentifierToTaxon(y);

	var comp = engine.makeSimComparisonResults([stuff]);
