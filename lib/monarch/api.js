    ...
    if (resultObj.results.length > 0){
        ...
        if (gene.references[0]){
            ...
            for (var i=0;i < gene.references.length; i++){
            	...
            	var modelObj = this.fetchModelOrgGenePhenotypeAsAssociation(gene.references[i],gene.label);
...
bbop.monarch.Engine.prototype.fetchModelOrgGenePhenotypeAsAssociation = function(id,label) {
    //Hard coding per source
    if (id.source == 'MGI'){
        var resource_id = 'nif-0000-00096-6'; //ID map
        var filter = ["implicated_gene_ids:"+this.quote(id.id)];
