...
bbop.monarch.Engine.prototype.fetchOrthologList = function(ids) {
    ...
    filter = "^WormBase|FlyBase|RGD|ENSEMBL\:(ENSMODG|ENSECAG|ENSECAG|ENSMMUG|" +
    		 "ENSCAFG|ENSBTAG|ENSSSCG|ENSPTRG|ENSACAG|ENSGALG)";
    filtered_regex = new RegExp(filter);
...
bbop.monarch.Engine.prototype.fetchGeneOrthologFromSciGraph = function(id) {
    ...
    var path = '/dynamic/genes/'+id+'/homologs.json';
    var scigraphResults = engine.querySciGraphDynamicServices(path,params);
    
    var orthologGraph = new bbop.model.graph();
    orthologGraph.load_json(scigraphResults);
    var node_list = orthologGraph.get_descendent_subgraph(id,'equivalentClass').all_nodes();

    var equivalent_classes = node_list.map( function(obj){ return obj.id(); }).concat(id);
    var results = [];
    
    orthologGraph.all_edges().forEach(function(edge){
        if (equivalent_classes.indexOf(edge.subject_id()) >= 0){
            var resultObj = {
                    'gene': {
                        'id': id,
                        'label': orthologGraph.get_node(id).label()
                    },
                    'ortholog' : { 
                        id : edge.object_id()
                    },
...
bbop.monarch.Engine.prototype.compareEntities = function(x,y) {
    ...
    for (var it=0; it<y.length; it++) { 
		...
		if (n != null) {			
			stuff.j = { id : target_id, label : n.lbl, type : engine.mapIdentifierType(target_id), id_list : yphenotype_ids};
		}
...
bbop.monarch.Engine.prototype.getGraphNodeByID = function(id) {
    ...
    var uri = this.config.scigraph_data_url+path;
    var resultObj = [];
...
bbop.monarch.Engine.prototype.getNodeTypes = function(id) {
	...
	var uri = engine.config.scigraph_url+path;
    var resultObj = engine.querySciGraphGraphServices(uri,params);
	var types = [];
