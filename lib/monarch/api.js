...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
 ...
 // PHENOTYPES
    ...
    if (resultObj.results.length > 0){
        ...
        // Diseases
        disease_associations = disease_associations.concat(resultObj.disease_assocs);
        gene.disease_associations = disease_associations;
    } else {
        //Gene is not from homo sapiens, map NCBI gene to appropriate ID
        var geneMap = this.mapNCBIGeneToSource(gene.id);
        console.warn("Mapping to model organism");
        if (geneMap.results[0]){
            gene.references.push(geneMap.results[0].references);
            
            //Check to make sure gene is the same for each result
            for (var i=0;i < geneMap.results.length; i++){
                if (geneMap.results[0].id != geneMap.results[i].id){
                    console.warn("Multiple mapped genes for NCBI ID");
                    console.warn("Using first mapped gene");
                }
            }
            var modelObj = this.fetchModelOrgGenePhenotypeAsAssociation(geneMap.results[0].id,
            		                                                    geneMap.results[0].source,
            		                                                    gene.label);
            if (modelObj.results.length > 0){
...
bbop.monarch.Engine.prototype.fetchModelOrgGenePhenotypeAsAssociation = function(id,source,label) {
    //Hard coding per source
    if (source == 'MGI'){
        var resource_id = 'nif-0000-00096-6'; //ID map
        var filter = ["implicated_gene_ids:"+this.quote(id)];
...
bbop.monarch.Engine.prototype.mapNCBIGeneToSource = function(id) {
    ...
    var filter = ["mapped_source:MGI","source:NCBIGene","id:"+this.quote(id)];
...
bbop.monarch.Engine.prototype.fetchGenePhenotypeAsAssociation = function(id) {
    ...
    var unique = {};
	if (id != null) {
      id = id.replace(/NCBIGene:/,'NCBI_Gene:');
    }
    ...
    for (var i=0; i<fetchPheno.results.length; i++){
        ...
        disease = {
                id : "monarch:disco/" + results.e_uid,
                type : "Association", 
                gene : { 
                   id : result.gene_id, 
                   label : result.gene_label },
        
                //disease information
                disease : {
                    id : result.phenotype_id,
                    label : result.phenotype_label, },
         
                references : references(result.publication_id),
        ...
        disease_associations = disease_associations.concat(disease);
    ...
    resultObj.disease_assocs = disease_associations;
    return resultObj;
...
bbop.monarch.Engine.prototype.fetchGeneInfoFromMyGene = function(id) {
    ...
    if (id.match(omim)){
        ...
        ret = this.fetchUrl(mygene_url + 'query?q=mim:' + geneID + "&species=human,mouse,rat,zebrafish," +
                            "fruitfly,worm&fields=entrezgene,name,symbol,_id,taxid,ensembl,map_location");
        ret = JSON.parse(ret);
        ...
        ret = this.fetchUrl(mygene_url + 'query?q=' + id + "&species=human,mouse,rat,zebrafish,fruitfly," +
                            "worm&fields=entrezgene,name,symbol,_id,taxid,ensembl,map_location");
        ret = JSON.parse(ret);
    }
