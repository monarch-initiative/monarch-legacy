...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
    ...
    if (gene.references[0]){
        ...
        //for (var i=0;i < 1; i++){
			if (!(/MGI|ZFIN|NCBIGene/.test(gene.references[i].id))) {
				engine.info("skipping lookup of "+gene.references[i].id);
				continue;
			}
...
//TODO remove OMIM, not genotype
    ...
    for (var i=0; i<resources.length; i++) {
        ...
        var trAssoc = function (r) {
            ...
            engine.info("REFS:"+JSON.stringify(references));
			var geno = engine.makeGenotype(r);
			engine.log("GENOTAXON: "+JSON.stringify(geno.taxon));
			engine.log("DEFAULTTAXON: "+JSON.stringify(default_taxon));
