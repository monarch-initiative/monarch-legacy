...
bbop.monarch.Engine.prototype.fetchGeneOrthologFromSciGraph = function(id) {
    ...
    var stripped_id = id.replace(/^NCBIGene:/,'');
    
    var orthologGraph = new bbop.model.graph();
    orthologGraph.load_json(scigraphResults);
    var node_list = engine.get_equivalent_classes(orthologGraph,stripped_id);
    var equivalent_classes = node_list.map( function(obj){ return obj._id; }).concat(stripped_id);
    var results = [];
    
    orthologGraph.all_edges().forEach(function(edge){
        if (equivalent_classes.indexOf(edge._subject_id) >= 0){
            var resultObj = {
                    'gene': {
                        'id': id,
                        'label': orthologGraph.get_node(stripped_id)._label
                    },
                    'ortholog' : { 
                        id : edge._object_id
                    },
                    
                    'orthology_class' : CLASS_MAP[edge._predicate_id]
            };
 ...
 *
 * Return a /copy/ list of nodes connected to an object and predicate
 *
 * Parameters:
 *  sub_id - the subject_id of the edge we're looking for
 *  pred - the predicate of the edge we're looking for
 *
 * Returns: 
 *  list of <bbop.model.node>
 */
bbop.monarch.Engine.prototype.get_subjects = function(graph, obj_id, pred){   

    var retlist = new Array();
    var node_list = graph.all_nodes();

    for (var i in node_list){
        if (graph._sop_table[node_list[i]['_id']]
            && graph._sop_table[node_list[i]['_id']][obj_id]
            && graph._sop_table[node_list[i]['_id']][obj_id][pred]){
            var tmp_node = graph._nodes.hash[node_list[i]['_id']];
            var node = tmp_node.clone();
            retlist.push(node);
        }
    }
    return retlist; 
...
bbop.monarch.Engine.prototype.get_equivalent_classes = function(graph, obj_id){   
    var engine = this;
    var retlist = new Array();
    var node_list = graph.all_nodes();
    ...
    for (var i in node_list){
        if (graph._sop_table[node_list[i]['_id']]
            && graph._sop_table[node_list[i]['_id']][obj_id]
            && graph._sop_table[node_list[i]['_id']][obj_id]['equivalentClass']){
            var equiv_list = 
                engine.get_equivalent_classes(graph,node_list[i]['_id']);
            var tmp_node = graph._nodes.hash[node_list[i]['_id']];
            var node = tmp_node.clone();
            retlist.push(node);
            retlist = retlist.concat(equiv_list);
        }
    }
    return retlist; 
};
...
bbop.monarch.Engine.prototype.querySciGraphDynamicServices = function(path,params) {
    ...
    //Fetch URI
    var uri = engine.config.scigraph_data_url;
