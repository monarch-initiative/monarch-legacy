...
bbop.monarch.Engine = function(opts) {
    ...
    if (this.config.scigraph_services_url == null) {
        this.config.scigraph_services_url = "http://rosie.crbs.ucsd.edu:9000/scigraph/";
    }
...
bbop.monarch.Engine.prototype.fetchGeneAssociations = function(id) {
    var url = this.config.scigraph_services_url + 'dynamic/genotypes_from_gene.json'
    var ret = this.fetchUrl(url + '?gene_id=' + id);
    ret = JSON.parse(ret);
    console.log(ret);
    return ret;
}
