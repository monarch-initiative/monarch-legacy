...
//come straight from the variantdisease mappings; perhaps even from >1 resource
    ...
    resources.forEach(function(resource) {
        ...
        var trDGA =
            function (r) {
                ...
                var disease_id = r.disease_id;
                var disease_label = r.disease_label;
                if (resource.id == 'nlx_151671-2') {
                    disease_id = r.omim_id;
                }
                var assoc = { disease : { id : disease_id, label : disease_label }, 
                              gene : {id : r.gene_id, label : r.gene_label}, 
                              references : [],
                              source : [],
                            };
                if (assocLookup[disease_id+r.gene_id]) {
                    assoc = assocLookup[disease_id+r.gene_id];
                } else {
                    assocLookup[disease_id+r.gene_id] = assoc;
                };
    ...
    var resultObj =
            engine.fetchDataFromResource(
                    ...
                    resource.id,
                    trDGA,
                    ['gene_id','gene_label','disease_id','disease_label','publication_label','publication_id','gene_ids','gene_labels','publication_ids','omim_id'],
                    filters,
                    null,
                    { useClosure : true}
                );
