...
bbop.monarch.Engine.prototype.fetchClassInfo = function(id, opts) {
    ...
    } else if (res.list instanceof Array && res.list[0]) {
		//was there a change in services?  concepts renamed to list?
        resultObj = res.list[0];
        resultObj.id = id;
        //TODO change this upstream
        if (resultObj.labels[0] != null){
            resultObj.label = resultObj.labels[0];
...
bbop.monarch.Engine.prototype.getGraphNodeByID = function(id) {
    ...
    var uri = engine.config.scigraph_data_url+path;
    var resultObj = [];
