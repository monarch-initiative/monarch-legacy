...
bbop.monarch.Engine.prototype.searchByPhenotypeProfile = function(query,target_species,target_type,cutoff,metric) {
    ...
    if (target.constructor === Array) {
        stuff = engine.searchByAttributeSet(id_list,target,cutoff);
        simResults = engine.makeSimComparisonResults(stuff.results,metric,target_type,target_species);
        similarThings.b = similarThings.b.concat(simResults.b);
    } else {
        stuff = engine.searchByAttributeSet(id_list,target,cutoff);
        similarThings = engine.makeSimComparisonResults(stuff.results,metric,target_type,target_species);
    }
 ...
 * 
 * Arguments: term : search term Returns: JSON structure - as per SciGraph vocabular
 * services
 */
...
bbop.monarch.Engine.prototype.getInitialGolrManager = function(personality) {
    ...
    var golrServer = this.config.golr_url;
