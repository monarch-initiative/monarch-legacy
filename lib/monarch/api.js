...
bbop.monarch.Engine.prototype.getInformationProfile = function(annotation_profile,categories) {
	...
	console.log("SCORE:"+JSON.stringify(all_info));
	...
	scored_profile.overall_score = parseFloat(att_info.annotation_sufficiency.score);
	console.log("SCORE:"+att_info.annotation_sufficiency.score);
	if (nots.length > 0) {
	...
	if (scores.length > 0) {
		...
		scored_profile.scaled_score = ((scored_profile.overall_score - min) 
			+ (scored_profile.categorical_score* category_scaling_factor)) / (1+category_scaling_factor);

	}

	//assume the categories are scaling factors for the minimum
	//it really should be done by the client.
...
bbop.monarch.Engine.prototype.fetchUrl = function(url, params, method) {
    ...
    if (exchangeObj.status != 200) {
		console.log(exchangeObj.toSource());
        throw({
            type : "fetchUrl",
            url: url,
