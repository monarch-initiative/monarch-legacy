 ...
 *
 * Arguments: term : search term Returns: JSON structure
 */
 bbop.monarch.Engine.prototype.autocomplete = function(term,categories,limit) {
    ...
    var golrServer = this.config.search_url;

    // TODO return only fields of interest
    var golrManager = new golr_manager(golrServer, golrConf, engine_to_use, 'sync');
    golrManager.set_personality("monarch_search");
    golrManager.include_highlighting(true);

    golrManager.set_query(term);
    golrManager.set_results_count(limit);

    console.log(golrManager.get_query_url());

    var r = golrManager.search();

    console.log(r.documents());
    var taxons = _.map(_.values(r.documents()), function(obj){
      if (obj.taxon != null || typeof obj.taxon != 'undefined') {
        return obj.taxon[0];
      } else {
        return "";
      }
    ...
    var category = _.map(_.values(r.documents()), function(obj){
      if(_.contains(obj.category, "Phenotype")) {
          return "phenotype";
      } else if(_.contains(obj.category, "disease")){
          return "disease";
      } else {
          return "gene";
      }
    ...
    var index = 0; // TODO kinda dirty
    var hl = _.mapObject(r._raw.highlighting, function(val, key) {
      var completion =  _.values(val)[0][0]; // first autocompleted field and first element of this array
      var json =  {
        "id": key,
...
bbop.monarch.Engine.prototype.getInitialGolrManager = function(personality, golrServer) {
    // Get configuration from self
    // Instantiated in webapp_launcher_*.js from conf/golr-conf.json
    var golrConf = new bbop.golr.conf(this.config.golr);
...
bbop.monarch.Engine.prototype.fetchAssociations = function(id, field, filter, limit, personality, facet) {
    ...
    var golrManager = engine.getInitialGolrManager(personality);
    ...
    var url = golrManager.get_query_url();
