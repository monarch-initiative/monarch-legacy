...
bbop.monarch.Engine.prototype.fetchSpotlight = function(type, id) {
    ...
    switch (type) {
	    ...
	    spotlight_data.description = info.description;
	    ...
	    if (typeof info.phenotype_associations != 'undefined') {
                info.phenotype_associations.map( function(a) {
		  spotlight_data.phenotypes.push(a.phenotype);
	    	});
	    }
	    ...
	    spotlight_data.heritability = info.heritability;
	    //engine.log("HERIT="+JSON.stringify(info.heritability));
	    if (info.heritability != null) {
		spotlight_data.heritability = engine.unique(info.heritability.map(function(h) { return h.inheritance}));
	    }
            spotlight_data.phenotypes = [];
            if (info.phenotype_associations != null) {
                info.phenotype_associations.forEach(function(p) {spotlight_data.phenotypes.push({id:p.phenotype.id,label:p.phenotype.label})});
            }
            spotlight_data.anatomy = []; //TODO - figure out anatomical parts
