...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
	...
	gene.pathways = this.fetchPathwaysForGene(gene.id).pathways;
...
//		'7995' : {id:'',label'zebrafish models'}
    ...
    var tr =
        function (r) {
			// these are properties of the container
            if (queryLookup[engine.getNifId(r.a)]) {
                query = queryLookup[r.a];
            }
            else {
				//var id = engine.getNifId(r.a);
                queryLookup[id] = {
					id : id,
...
//      But it could be genes or other feature types.
	...
	var species_to_filter_map = {
			...
			'7995' : { target_idspace : 'ZFIN', b_type : 'gene'}
	};
...
bbop.monarch.Engine.prototype.mapGeneToNCBIgene = function(id) {
    ...
    var resultObj = this.fetchDataFromResource(null, resource_id, tr,null,["source:NCBI_gene","mapped_id:"+this.getNifId(id)],null,null,{ useClosure : false});
	...
	if (resultObj.results == 0 ) {
		...
		var newid = idsplit[2];
		if (idsplit != null) {
			var resultObj = this.fetchDataFromResource(null, resource_id, tr,null,["source:NCBI_gene","mapped_id:"+this.getNifId(newid),"mapped_source:"+idspace],null,null,{ useClosure : false});	
		}
	}
...
bbop.monarch.Engine.prototype.mapNCBIGeneToKO = function(id) {
    ...
    var ncbiToKeggGene = this.fetchDataFromResource(null, id_map_resource_id, tr,null,["source:NCBI_gene","id:"+id,"mapped_source:KEGG"],1,null,{ useClosure : false});
	console.log("NCBI:KEGG:"+JSON.stringify(ncbiToKeggGene));
	var kegg_geneid;
	if (ncbiToKeggGene.results.length == 0 ) {
		kegg_geneid = null; 
    } else {
		kegg_geneid = this.getNifId(ncbiToKeggGene.results[0].mapped_id.replace("KEGG:",""));
	}
	...
	var keggGeneToKO = this.fetchDataFromResource(null,kegg_ko_map_resource_id, keggtr, null,["gene_id:"+kegg_geneid],null,null,{useClosure : false});
...
bbop.monarch.Engine.prototype.searchSubstring = function(term) {
    ...
    var res= this.fetchUrl(this.config.autocomplete_url+"?prefix="+term);
    console.log("substring res length  is "+res);
    // parsing it out to JSON may be a bit inefficient,
    // as it is a pass-through to the webapp call that will immediatly turn it back
    // into a string.
    // However, this is consistent with the rest of the API.
