...
bbop.monarch.Engine.prototype.fetchSpotlight = function(type, id) {
	...
	//return a random one
	var index=Math.round(Math.random()*(spotlight_config[type].length));
...
bbop.monarch.Engine.prototype.fetchDiseaseGeneAssociations = function(id,type,returnResultCount) {
...
bbop.monarch.Engine.prototype.fetchClassInfo = function(id, opts) {
	...
	var res = engine.getVocabularyByID(id,tr);
	//TODO do we need to handle lists of >1
	resultObj = res[0];
	resultObj.database_cross_reference = engine.getSciGraphXRefs(id);
    ...
    if (opts != null) {
        if (opts.level != null && opts.level > 0) {
            ...
            var rel = engine.getGraphNeighbors(id,opts.level,tr);
...
bbop.monarch.Engine.prototype.searchSubstring = function(term, opts) {
    ...
    var tr = function (r) {
        ...
        //TODO remove when categories implemented
    	/* if (/NCBIGene/.test(r.concept.uri) && (!obj.category)){
    		obj.category = 'gene';
    	} else if (/DOID/.test(r.concept.uri) && (!obj.category)){
    		obj.category = 'disease';
    	}
    	if (!obj.category || !(/gene|phenotype|disease/i).test(obj.category) ){
    		return [];
    	} else {
    	    return obj;
    	} */
    ...
    var results = engine.getVocabularyByTerm(term,searchType,limit,categories,searchSynonyms,null,tr);
    var filtered = [];
...
bbop.monarch.Engine.prototype.searchOverOntologies = function(term, opts) {
    ...
    var tr = function (r) {
        ...
        //TODO remove after testing categories
    	/*if ( /NCBIGene/.test(obj.id)&&(!obj.category)){
    		obj.category = 'gene';
    	} else if ( /DOID/.test(obj.id)&&(!obj.category)){
    		obj.category = 'disease';
    	}
    	if (!obj.category){
    		return [];
    ...
    results = engine.getVocabularyByTerm(term,searchType,limit,categories,searchSynonyms,null,tr);
...
bbop.monarch.Engine.prototype.getVocabularyByTerm = function(term,searchType,limit,category,searchSynonyms,ontology,trFunction) {
    ...
    var resultObj = [];
    resultObj = engine.querySciGraph(uri,params,trFunction);
    return resultObj;
}
...
bbop.monarch.Engine.prototype.getSciGraphXRefs = function(id) {
    ...
    resultObj = engine.querySciGraph(uri);
...
bbop.monarch.Engine.prototype.querySciGraph = function(uri,params,trFunction) {
	...
	//Fetch URI
    try {
        resultStr = engine.fetchUrl(uri,params);
        ...
        if (sciObj instanceof Array){
            sciObj.forEach(function(r) {
                ...
                if (result instanceof Array){
                    resultObj.push.apply(resultObj,result);
                } else {
        	...
        	if (sciObj.list){
        		...
        		sciObj.paths.forEach(function(r) {
                    var result = trFunction(r);
                    if (result instanceof Array){
                        resultObj.push.apply(resultObj,result);
                    } else {
