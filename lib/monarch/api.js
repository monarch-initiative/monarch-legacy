...
bbop.monarch.Engine.prototype.fetchTaxon = function(id) {
    ...
    var leader = engine.fetchCliqueLeader(id, engine.config.scigraph_data_url_backend);
    var neighbors = engine.getGraphNeighbors(leader, 1, 'RO:0002162', 'BOTH', false, engine.config.scigraph_data_url_backend);
...
bbop.monarch.Engine.prototype.fetchInheritance = function (id) {
    ...
    const neighbors = engine.getGraphNeighbors(
        ...
        params.blankNodes, engine.config.scigraph_data_url_backend
    );
...
bbop.monarch.Engine.prototype.compareEntities = function(x,y) {
    ...
    for (var it=0; it<y.length; it++) {
        ...
        var node = engine.getGraphNodeByID(target_id, engine.config.scigraph_data_url_backend);
        var graph = new bbop.model.graph();
    ...
    var a = {};
    if (x.length == 1) {
        var n = engine.getGraphNodeByID(x[0], engine.config.scigraph_data_url_backend);
        var graph = new bbop.model.graph();
        graph.load_json(n);
        ...
        x.forEach( function (xid) {
            var n = engine.getGraphNodeByID(xid, engine.config.scigraph_url_backend);
            var graph = new bbop.model.graph();
            graph.load_json(n);
...
bbop.monarch.Engine.prototype.getCategories = function(id) {
    var engine = this;
    var categories = [];
    var nodeByID = engine.getGraphNodeByID(id, engine.config.scigraph_data_url_backend);
    var graph = new bbop.model.graph();
...
bbop.monarch.Engine.prototype.annotateText = function(txt, opts) {
    ...
    var annotate_url = this.config.scigraph_url_backend + "annotations/entities.json";
    var res =
    ...
    var neighbors = engine.getGraphNeighbors(id_list, 1, 'RO:0002162', 'BOTH', false, engine.config.scigraph_data_url_backend);
    var graph = new bbop.model.graph();
    graph.load_json(neighbors);
...
bbop.monarch.Engine.prototype.search = function(term, filters, facets, rows, start) {
    ...
    var searchGolrManager = engine.initializeGolrManager("monarch_search", this.config.search_url_backend);
    ...
    var cat_filter = categories.join(" OR ");
    searchGolrManager.add_query_filter("category", `(${cat_filter})`);
    ...
    var hl = _.map(zipped, function(tuple) {
        var json = tuple[0];
        var reduced_cat = json.category.filter(val => categories.includes(val));
        json.category = reduced_cat;
        json.category_std = reduced_cat;
    ...
    var category_facets = r._raw.facet_counts.facet_fields.category;
    var reduced_facets = category_facets.filter(val => categories.includes(val[0]));
    r._raw.facet_counts.facet_fields.category = reduced_facets;
...
bbop.monarch.Engine.prototype.searchOverOntologies = function(term, opts) {
    ...
    try {
        var neighbors = engine.getGraphNeighbors(id_list, 1, 'RO:0002162',
                'BOTH', false, engine.config.scigraph_url_backend);
        var graph = new bbop.model.graph();
        graph.load_json(neighbors);
    ...
    var eq_graph = engine.getGraphNeighbors(id_list, 5, 'equivalentClass', 'BOTH', false, engine.config.scigraph_url_backend);
...
bbop.monarch.Engine.prototype.getGraphNodeByID = function(id, server) {
    ...
    if (!server){
        server = engine.config.scigraph_url_backend;
    }
...
bbop.monarch.Engine.prototype.querySciGraphDynamicServices = function(path,params) {
    ...
    //Fetch URI
    var uri = engine.config.scigraph_data_url_backend+'dynamic/';
...
bbop.monarch.Engine.prototype.initializeGolrManager = function(personality, golrServer) {
    ...
    if (typeof(golrServer) === 'undefined') golrServer = this.config.golr_url_backend;
...
bbop.monarch.Engine.prototype.fetchCliqueLeader = function(id, server) {
    ...
    if (!server){
        server = engine.config.scigraph_data_url_backend;
    }
...
bbop.monarch.Engine.prototype.fetchDataInfo = function(id, opts) {
    ...
    var resultObj = {};
    var engine = this;
    var server = engine.config.scigraph_data_url_backend;
...
bbop.monarch.Engine.prototype.getHealthCheck = function () {
    var engine = this;
    var health_status = {};
    var services = ['scigraph_url_backend', 'scigraph_data_url_backend', 'golr_url_backend', 'owlsim_services_url'];
    ...
    try {
        engine.getGraphNodeByID('DOID:4', engine.config.scigraph_url_backend);
        health_status.scigraph_url_backend.status = 'pass';
    } catch (e){
        health_status.scigraph_url_backend.status = 'fail';
    }

    // Test scigraph data
    try {
        engine.getGraphNodeByID('DOID:4', engine.config.scigraph_data_url_backend);
        health_status.scigraph_data_url_backend.status = 'pass';
    } catch (e){
        health_status.scigraph_data_url_backend.status = 'fail';
    }

    // Test golr
    try {
        engine.fetchPhenotypes("OMIM:606693");
        health_status.golr_url_backend.status = 'pass';
    } catch (e){
        health_status.golr_url_backend.status = 'fail';
    }
    // Test owlsim
    try {
...
bbop.monarch.Engine.prototype.getGolrDownloadUrl = function (id, format, association) {
    ...
    var golrServer = engine.config.golr_url_backend;
