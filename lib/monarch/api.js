            ...
            for (var i = 0; i < phenotype_associations.length; i += 1) {
                ...
                if (ref != null) {
                    for (var j = 0; j < ref.length; j += 1) {
                        var currRef = ref[j];
                        var str = currRef.id.substring(0, 5);
                        var num = currRef.id.substring(5);
                        if (str.equals("PMID:") || str.equals("pmid:")) {
        ...
        if (alleles != null) {
            for (var i = 0; i < alleles.length; i += 1) {
                ...
                var ref = alleles[i].evidence.split(", ");
                    if (ref != null) {
                    for (var j = 0; j < ref.length; j += 1) {
                        var str = ref[j].substring(0, 5);
                        var num = ref[j].substring(5);
                        if (str.equals("PMID:") || str.equals("pmid:")) {
                            if (pmids.indexOf(num) == -1) {
                                pmids.push(num);
                            }
...
bbop.monarch.Engine.prototype.fetchPhenotypeInfo = function(id, opts) {
    ...
    if (obj.disease_associations != null) {
        for (var i = 0; i < obj.disease_associations.length; i += 1) {
            ...
            if (ref != null) {
                for (var j = 0; j < ref.length; j += 1) {
                    var str = ref[j].id.substring(0, 5);
                    var num = ref[j].id.substring(5);
                    if (str.equals("PMID:") || str.equals("pmid:")) {
                        if (pmids.indexOf(num) == -1) {
                            pmids.push(num);
                        }
    ...
    if (obj.genotype_associations != null) {
        for (var i = 0; i < obj.genotype_associations.length; i += 1) {
            ...
            if (ref != null) {
                var str = ref.id.substring(0, 5);
                var num = ref.id.substring(5);
                if (str.equals("PMID:") || str.equals("pmid:")) {
                    if (pmids.indexOf(num) == -1) {
                        pmids.push(num);
                    }
...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
    ...
    //Include only associations from query gene ID
    var checkDiseaseAssocs = function (id,disease_assocs){
    	var gene_disease_associations = [];
        for (var i=0;i < disease_assocs.length; i++){
        	if (disease_assocs[i].gene.id == id){
        		gene_disease_associations = 
        			gene_disease_associations.concat(disease_assocs[i])
        	}
        }
        return gene_disease_associations;
    }
    
    disease_associations = checkDiseaseAssocs(gene.id,disease_associations);
    ...
    if ((!gene.disease_associations[0])&&(gene.orthologs)&&(gene.taxon)&&(gene.taxon.id != 9606)){
    	for (var i=0;i < gene.orthologs.length; i++){
    		if (gene.orthologs[i].organism == 'Homo sapiens'){
    			...
    			if (disObj){
    				disease_ortho_assocs = checkDiseaseAssocs(gene.orthologs[i].ortholog.id,disObj);
...
bbop.monarch.Engine.prototype.fetchGeneAlleleAsAssociation = function(id,source, genObj) {
    ...
    if ((source == 'human')||(source == 'WB')||(source == 'MGI')||(source == 'ZFIN')){
        ...
        if (source == 'human'){
            tr =
              ...
              var obj = {
                ...
                references : [
                 /*
					 * Commenting out for now, are these evidence for
					 * gene-allele or allele-phenotype? { id : r.publication_id
					 * //labels are too large for table //label :
					 * r.publication_label }
					 */
                 ],
        ...
        /*
		 * var resultObj2 = this.fetchDataFromResource(null, resource_id,
		 * trZFIN, null, filter, null, null, { useClosure : false});
		 */	
        	if (resultObj1.results){
            	resultObj.results = resultObj.results.concat(resultObj1.results);
            }
        /*
		 * if (resultObj2.results){ resultObj.results =
		 * resultObj.results.concat(resultObj2.results); }
		 */
...
bbop.monarch.Engine.prototype.fetchGenotype = function(id,sp) {
    var species_to_resource_map = {  // HARDCODE ALERT
        ...
        '9606' : {id: 'nif-0000-03216-9',label:'OMIM'}    // OMIM - this is a
															// variant table
    };
...
// But it could be genes or other feature types.
    ...
    var species_to_filter_map = {
            ...
            '7227' : { target_idspace : 'FBgenoInternal', b_type : 'genotype'},
    ...
    var target = species_to_filter_map[target_species].target_idspace;  // HARDCODE
																		// ALERT
    ...
    // TODO: what's odd here is that i'm postumously assigning rank, but i don't
	// know what the sort order is!
    ...
    var similarThings = {
            ...
            b : results,  // TODO: process results to select only a single
							// metric, reformat, etc.
            cutoff : cutoff,
            // TODO: this metadata should be an owlsim call, or perhaps returned
...
bbop.monarch.Engine.prototype.mapNCBIGeneToKO = function(id) {
    ...
    //if (kegg_geneid != null){
    	...
    	var keggGeneToKO = this.fetchDataFromResource(null,kegg_ko_map_resource_id, keggtr, null,["gene_id:"+this.quote("NCBIGene:"+id)],null,null,{useClosure : false});
 ...
 * Example Output: [{"x":"homo sapiens","y":"16"},{"x":"c elegans","y":"18"}]
 */
bbop.monarch.Engine.prototype.generateHistogram = function() {
	//Check cache
	if (this.cache != null) {
    ...
    //HP classes, harcoded for now, should be switched to scigraph query
    var categories = engine.config.summary_categories;
    var hp_labels = {
    ...
    categories.forEach(function(category) {
        ...
        resources.forEach(function(resource) {
    		var url = urlBase+resource.id+".json?q="+category+opts;
    		var resultObj = JSON.parse(engine.fetchUrl(url));
    		if (resultObj.result){
    		    var resultCount = resultObj.result.resultCount;
    		    var d3Bar = {
    				       "value" : resultCount,
    				       "name" : resource.label
    		            };
