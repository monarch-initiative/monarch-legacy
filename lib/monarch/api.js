...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
    ...
    if (id.match(/MGI/)&&(!gene.id)) {
        ...
        // Fixes issue where these cases generate all diseases fetchDiseaseGeneAssociations
        if (disease_associations.length >= 3140){
...
bbop.monarch.Engine.prototype.fetchGeneAlleleAsAssociation = function(id,source, genObj) {
    var source_to_resource_map = {    //HARDCODE ALERT
            ...
            'ZFIN'  : {id:'nif-0000-21427-11',label:'ZFIN'}, //ZFIN
            'ZFIN2'  : {id:'nif-0000-21427-12',label:'ZFIN'}
         };
    var resultObj = {};
    resultObj.results = [];
    //supported resources
    if ((source == 'human')||(source == 'WB')||(source == 'MGI')||(source == 'ZFIN')){

            var resource_id = source_to_resource_map[source].id;
            var filter;
            
            var formattedID = id.replace(/NCBI_?[Gg]ene:/,'NCBI_gene:');
            formattedID = formattedID.replace(/WB:/,'');
            
            if ((source == 'human')||(source == 'WB')||(source == 'MGI')){
        	    filter = ["gene_id:"+this.quote(formattedID)];
            }
        ...
        if (source == 'human'){
            tr =
              ...
              r.sequence_alteration_label = 
            	  r.sequence_alteration_label.replace(/[<>]/g,'');
              
              r.sequence_alteration_id = 
            	  r.sequence_alteration_id.replace(/\-SA$/,'');
              r.sequence_alteration_id = 
            	  r.sequence_alteration_id.replace(/_/,'#');
        ...
        } else if ((source == 'MGI')||(source == 'ZFIN')){
        	tr =
                function (r) {
                ...
                var obj = {
                  ...
                  variant_type : r.sequence_alteration_type_label,
                  references : [
                   /*Commenting out for now, 
                    * are these evidence for gene-allele or allele-phenotype?
                    *  {
        	...
        	var trZFIN =
                ...
                var obj = {
                  ...
                  variant : { 
                      id : r.targeted_gene_variant_id,
                      label : r.targeted_gene_variant_label
                  },
                  variant_type : '',
                  references : [
                   /*Commenting out for now, 
        	...
        	tr =
                ...
                var obj = {
                  ...
                  variant_type : r.sequence_variation_type,
                  references : [
                   /*Commenting out for now, 
                    * are these evidence for gene-allele or allele-phenotype?
                    *  {
        ...
        if (source == 'ZFIN'){
        	...
        	resource_id = source_to_resource_map['ZFIN2'].id;
        ...
        */
        ...
        if (resultObj){
        	for (var i=0; i<resultObj.results.length; i++){
                var cat = resultObj.results[i].variant.id;
        		...
        		if (cat in unique){
                	...
                	//to the query gene
                	if (resultObj.results[i].variant.id.match(/\,/)){
                		continue;
                	}
                	if (resultObj.results[i].variant.id){
...
bbop.monarch.Engine.prototype.fetchGenePhenotypeAsAssociation = function(id) {
    ...
    var unique = {};
	if (id != null) {
      // NIF production still as NCBI_[Gg]ene:
      //id = id.replace(/NCBI_[Gg]ene:/,'NCBIGene:');
		id = id.replace(/NCBI[Gg]ene:/,'NCBI_gene:');
    }
...
bbop.monarch.Engine.prototype.resolveClassId = function(id) {
    ...
    if (cls.id.indexOf("DOID") == 0) {
        cls = this.fetchClassInfo(id,{level:1});
