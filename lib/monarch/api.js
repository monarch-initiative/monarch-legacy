...
bbop.monarch.Engine.prototype.fetchPhenotypeInfo = function(id, opts) {
    ...
    if (disease_associations != null) {
        for (var i = 0; i < disease_associations.length; i += 1) {
            var disease = disease_associations[i].disease;
            var ref = disease_associations[i].references;
            if (ref != null) {
                for (var j = 0; j < ref.length; j += 1) {
    	...
    	for (var i=0; i<resultObj.length; i++){
    		...
    		if (row in unique){
    			...
    			if (isSourceNew == 'true' && resultObj[i].source[0].id){
    				...
    				uniqResults[ifetndex].source = uniqSource;
    		    }
