...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
    ...
    var fetchOrg = 
    	//HACK - TODO need to find a better way to fetch correct nif results
...
bbop.monarch.Engine.prototype.annotateText = function(txt, opts) {
    ...
    var res = 
        this.fetchUrl("http://neuinfo.org/servicesv1/v1/annotate/complete.json", q);
...
bbop.monarch.Engine.prototype.searchSubstring = function(term) {
	...
	for (var j =0; j < filtered.length; j++) {
	    if (results[i].id === filtered[j].id || results[i].term.toLowerCase() === filtered[j].term.toLowerCase()) {
		found = true;
		break;
	    }
	}
