...
bbop.monarch.Engine.prototype.makeInferredGenePhenotypeAssociations = function(assocs) {
    ...
    if (assocs != null) {
        for (var i = 0; i < assocs.length; i += 1) {
            var assoc = assocs[i];
			var these_genes = {};
            console.info("ASSOC:"+JSON.stringify(assoc));
			...
			for (var gene in these_genes) {
				console.log("Expanding association to gene "+gene);
				var new_assoc = {};
	...
	console.log('created '+gene_assocs.length+' associations');
...
bbop.monarch.Engine.prototype.fetchGenotypeInfo = function(id, opts) {
	...
	if (obj == null) {
		...
		console.log("fly genotype found")
	}
...
bbop.monarch.Engine.prototype.fetchOmimDiseasePhenotypeAsAssocations = function(id, opts) {
	...
	subclassFilters['phenotype_id'] = id;
        ...
        console.warn("Assuming ID is a phenotype: "+id);
	subclassFilters['phenotype_id'] = id;
    }
    filters["aspect"] = "O";
...
bbop.monarch.Engine.prototype.generateHistogram = function(id,levels,isCached,filter) {
    ...
    Object.keys(hp_labels).forEach(function(category) {
        ...
        resources.forEach(function(resource) {
    	    ...
    	    /*var resultObj = engine.fetchDataFromResource(null,resource.id,
    	                                               null,null,
    					                               subclassQuery);*/
    		var resultObj={};
    		try {
    			//resultObj = JSON.parse(engine.fetchUrl(url));
    			resultObj = engine.fetchDataFromResource(null,resource.id,
                        null,null,
                        subclassQuery);
    		}
    		catch(err){
    			console.log("caught exception "+err+"for "+resource.id);
    			var d3Bar = {
				         "value" : 0,
				         "name" : resource.label
		                };
