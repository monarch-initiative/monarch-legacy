 ...
 */
bbop.monarch.Engine.prototype.getPhenotypeDistro = function(id,levels,isCached,filter) {
	//Check cache
	if (this.cache != null) {
...
bbop.monarch.Engine.prototype.getDiseaseGeneDistro = function(id,levels,isCached,filter) {
	var engine = this;
	//Check cache
	if (this.cache != null) {
    ...
    Object.keys(subClasses).forEach(function(diseaseClass) {
    	...
    	try {
    		geneCount = engine.fetchDiseaseGeneAssociations(diseaseClass,'disease',true);
    	}
    	catch(err){
    		console.error("caught exception "+err+"for "+resource.id);
    		var d3Bar = {
