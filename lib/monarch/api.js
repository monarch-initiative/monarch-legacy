...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
	...
	//TODO necessary?
    var unique = {};
    for (var k=0; k < gene.xrefs.length; k++){
    	unique[gene.xrefs[k].id] = 1;
    }
	...
	if (gene.taxon.id == 'NCBITaxon:9606') {
    ...
    if (gene.xrefs[0]){
        ...
        for (var i=0;i < gene.xrefs.length; i++){
			var g = gene.xrefs[i];
			if (!(/FB|MGI|ZFIN|NCBIGene/.test(g.id))) {
            ...
            var orthoObj = engine.fetchGeneOrthologAsAssociation(gene.xrefs[i].id,gene.label,gene);
    ...
    if (id.match(/MGI/)&&(!gene.ncbi_id)) {
        ...
        var orthoObj = engine.fetchGeneOrthologAsAssociation(formID,'MGI',gene);

        var alleleObj = engine.fetchGeneAlleleAsAssociation(formID,gene,10090);
...
bbop.monarch.Engine.prototype.filterXrefsForGene = function(geneMap) {
...
bbop.monarch.Engine.prototype.fetchDiseaseGeneAssociations = function(id,type) {
    ...
    var uniqResults = engine.collapseEquivalencyClass(results,["gene"],'disease',true);
    return uniqResults;
...
bbop.monarch.Engine.prototype.makeGenotype = function(r) {
	...
	if (typeof r.gene_id != 'undefined' && typeof r.implicated_gene_ids != 'undefined') {
		...
		//TODO refactor
		geno_obj.has_affected_genes = [{id:r.gene_id,label:r.gene_label}];
	}
    ...
    //this.log("CACHE " + JSON.stringify(geno_obj, null, ' '));
    return geno_obj;
};
...
bbop.monarch.Engine.prototype.fetchGenotype = function(id,sp) {
    ...
    var tr =
        function (r) {
			...
			//do this in case some old cached results don't have the species
            if (geno.taxon == {} || typeof geno.taxon == 'undefined' || geno.taxon == null || geno.taxon.id == '') {
...
//TODO remove OMIM, not genotype
    ...
    for (var i=0; i<resources.length; i++) {
		...
		if (type == 'phenotype') {
			...
			resultObj = engine.fetchDataFromResource(id,resource.id,trAssoc,null,null,resource.filter);
		}
		all_results = all_results.concat(resultObj.results);
...
bbop.monarch.Engine.prototype.getEquivalentClassList = function(id,level) {
    ...
    if (id){
        var rel = engine.getGraphNeighbors(id,level,tr);
    } else {
    	return null;
    }
