    ...
    for (var i=0; i<disease_ids.length; i++) {
        ...
        if (alleles != null) {
            for (var i = 0; i < alleles.length; i += 1) {
				...
				if (typeof alleles[i].evidence != 'undefined') {
...
bbop.monarch.Engine.prototype.trVariant = function(r,variant_type) {
	...
	if (typeof variant.id != 'undefined') {
		...
		variant.coordinates = {};
        if (typeof r.cytogenetic_location != 'undefined' && r.cytogenetic_location != '') {
			variant.coordinates.band = r.cytogenetic_location;
		}
...
bbop.monarch.Engine.prototype.fetchModelOrgGenePhenotypeAsAssociation = function(id,source,genObj) {
    ...
    // TODO integrate flybase data
	var resource = source_to_resource_map[source];
...
bbop.monarch.Engine.prototype.fetchGenoPhenoAsAssociationsBySpecies = function(id,sp,type) {
    ...
    var species_to_resource_map = {
        '10090' : {resources : [{id:'nif-0000-00096-6',label:'MGI'},{id:'nif-0000-03160-2',label:'MPD',filter:{nstdev_abs : '>3.0'}}], default_taxon : {id : 'NCBITaxon:10090',label : 'Mus musculus'}},
...
//TODO remove OMIM, not genotype
    ...
    for (var i=0; i<resources.length; i++) {
		...
		if (type == 'phenotype') {
			...
			resultObj = engine.fetchDataFromResource(id,resource.id,tr,null,null,resource.filter);
		}
...
bbop.monarch.Engine.prototype.makePublicationList = function(r) {
	...
	if (r.publication_ids != null && typeof r.publication_ids != 'undefined' && r.publication_ids != '') {
		ids = ids.trim();
		ids = ids.split(/[,;]/);
