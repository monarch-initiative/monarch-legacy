    ...
    var meta =
        {
            ...
            config : this.config,
            cacheContents : (this.cache != null ? this.cache.contents() : [])
...
/*
    var pheno_features = phenotype_associations.map(function(p) { return {  id : p.phenotype.id, 
          "isPresent" : (p.modifier == "normal" ? "false" : "true") } });  
    var annotation_profile = { id : obj.id, label : obj.label, features : pheno_features };
...
*/
	...
	phenotype_associations.annotation_sufficiency = engine.getAnnotationSufficiencyScoreFromAssociations(phenotype_associations);
...
bbop.monarch.Engine.prototype.getAnnotationSufficiencyScoreFromAssociations = function(assocs) {
	...
	//assoc style not uniform.  sometimes it is p.phenotype.id; other times it is in has_phenotype
...
bbop.monarch.Engine.prototype.fetchGenotypeInfo = function(id, opts) {
    ...
    obj.phenotype_associations.annotation_sufficiency = engine.getAnnotationSufficiencyScoreFromAssociations(obj.phenotype_associations);
...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
    ...
    if (obj.source == "NOT FOUND") {
        ...
        if (obj.ensembl){
            if (obj.ensembl.gene){
                gene.references.push({id:'Ensembl:'+obj.ensembl.gene});
            }
        }
    }
    ...
    // This code is generate associations for tables using ids other than NCBI gene
    ...
    // Generate orthologs
    var orthoObj = this.fetchGeneOrthologAsAssociation(id);
 ...
 * 
 */
bbop.monarch.Engine.prototype.fetchGeneOrthologAsAssociation = function(id) {
	var resource_id = 'nlx_152525-14'; //HARDCODE 
	
	var engine = this;
	var trimID;
	if (id != null) {
        trimID = id.replace(/NCBI_?[Gg]ene:/,'');
	}
	//This will break when NCBI_gene is updated
    var filter = ["human_gene_id_ncbi:" +this.quote("NCBI_gene:" + trimID)];

    var tr =
        function (r) {
    	
    	if (r.model_gene_id.match(/^MGI:MGI:/)){
            r.model_gene_id = r.model_gene_id.replace(/MGI:MGI:/,'MGI:');
        } 
    	...
    	var obj = {
                ...
                organism :  r.taxon_name,  
                orthology_class : r.orthology_class,
                
                // provenance
                source : {
                     id : r.orthology_source,
                     label : r.orthology_source
                 },
    		...
    		if (resultObj.results[i].ortholog.id){
    		    ...
    		    if (ncbigene_ids[0]){
    		        ...
    		        if (ncbiGene in unique){
    		        	//toss it out (although seeing different classes/source for same genes
                        continue;
                    } else {
 ...
 * Arguments:
 * - id : single gene id
 * - source : source of id (eg MGI)
 * - limit : object containing id and label of the linked gene
 * 
 */
bbop.monarch.Engine.prototype.fetchGeneAlleleAsAssociation = function(id,source, genObj) {
    var source_to_resource_map = {    //HARDCODE ALERT
            'human'  : {id:'nif-0000-03216-9',label:'OMIM'},   //OMIM
    ...
    if (source == 'human'){
            ...
            tr =
              ...
              var obj = {
                ...
                allele : { 
                  ...
                  * are these evidence for gene-allele or allele-phenotype?
                  *  {
                  *      id : r.publication_id
                  *      //labels are too large for table
        ...
        if (resultObj){
        	for (var i=0; i<resultObj.results.length; i++){
                var cat = resultObj.results[i].allele.id+
                          resultObj.results[i].phenotype.id;
             
        		if (cat in unique){
                    continue;
                } else {
                	if (resultObj.results[i].allele.id){
...
bbop.monarch.Engine.prototype.fetchGeneGenoTypeAsAssociation = function(id,source, genObj) {
	var source_to_resource_map = {    //HARDCODE ALERT
            ...
            'FB'   : {id:'nif-0000-00558-1',label:'FB'},    //FB
        ...
        if (resultObj){
        	for (var i=0; i<resultObj.results.length; i++){
                var cat = resultObj.results[i].phenotype.id+
                          resultObj.results[i].genotype.id;
...
bbop.monarch.Engine.prototype.fetchModelOrgGenePhenotypeAsAssociation = function(id,source,genObj) {
    ...
    var source_to_resource_map = {    //HARDCODE ALERT
            ...
            'ZFIN' : {id:'nif-0000-21427-10',label:'ZFIN'}, //ZFIN
            'WB'   : {id:'nif-0000-00053-4',label:'WB'},    //WB
            'FB'   : {id:'nif-0000-00558-2',label:'FB'},    //FB
...
bbop.monarch.Engine.prototype.fetchOmimDiseasePhenotypeAsGeneAssocations = function(id,genObj) {
...
bbop.monarch.Engine.prototype.fetchGeneInfoFromMyGene = function(id) {
    ...
    if (id.match(omim)){
        ...
        ret = this.fetchUrl(mygene_url + 'query?q=mim:' + geneID + "&species=human,mouse,rat,zebrafish," +
                            "fruitfly,nematode&fields=entrezgene,name,symbol,_id,taxid,ensembl,map_location");
        ret = JSON.parse(ret);
    } else if(id.match(mgi)){
        var geneID = id.replace("MGI:","MGI\\\\:");
        ...
        ret = this.fetchUrl(mygene_url + 'query?q=' + id + "&species=human,mouse,rat,zebrafish,fruitfly," +
                            "nematode&fields=entrezgene,name,symbol,_id,taxid,ensembl,map_location");
        ret = JSON.parse(ret);
    }
        ...
        var trDGA =
            function (r) {
                ...
                if (resource.label == 'ClinVar') {
					//TODO once UMLS gets added to uberdisease, then we can switch back
					...
					if (r.omim_ids.split(",").length == 1) {
						//TODO once KEGG and ORPHANET disease ids get put into uberdisease, we can use disease_id,
						//but i only want to do those that map 1:1 with disease_id - otherwise i might get suprious results
						...
						//TODO fix data scrubbing here; similarly, we only want to fetch the genes if there's only one OMIM id for kegg things
						if (genes) {
						...
						//remove the genes, we only want to map the genes if there's one mapped disease for kegg
						genes = [];
					}
				} else {
				...
				for (var j=0; j<diseases.length; j++) {
					...
					for (var i=0; i<genes.length; i++) {
						...
						//assoc.source = engine.unique(assoc.source);
 ...
 * Phenotips spec: http://phenotips.org/JSON+Sample
 * annotation_profile = {
 *  {  "id": "P0000001",
       "features":   [
            {
 ...
 *  where isPresent = true|false, where false are "not"-style annotations of
...
bbop.monarch.Engine.prototype.getInformationProfile = function(annotation_profile,categories) {
    ...
    //if (nots.length > 0) {
        //the NOTs will count only a fraction of the whole score
            ...
            //+ (parseFloat(not_info.annotation_sufficiency.score) * not_scaling_factor)) / (1+not_scaling_factor);
    //}

    //take a simple average of the categorical scores, regardless of their not status
    ...
    if (scores.length > 0) {
        ...
        //"curving" based on category minimum scores - Not sure if this is necessary
        //ommiting for now
        //var categorical_mins = engine.getMinScorePerCategory(categories);
        //var min = categorical_mins.map(function(x) { return parseFloat(x.score)} ).reduce(function(previous,current){ 
...
bbop.monarch.Engine.prototype.mapGeneToNCBIgene = function(id) {
    ...
    var resultObj = this.fetchDataFromResource(null, resource_id, tr,null,["source:NCBIGene","mapped_id:"+this.quote(this.getFederationNifId(id))],null,null,{ useClosure : false});
    ...
    if (resultObj.results == 0 ) {
        ...
        var newid = idsplit[2];
        if (idsplit != null) {
            var resultObj = this.fetchDataFromResource(null, resource_id, tr,null,["source:NCBIGene","mapped_id:"+this.quote(this.getFederationNifId(newid)),"mapped_source:"+idspace],null,null,{ useClosure : false});    
        }
    }
...
bbop.monarch.Engine.prototype.fetchPathwaysForGene = function(id) {
    ...
    var evidence_map_resource_id = 'nlx_31015-2'; //gene-to-pmid map //HARDCODE
    var engine = this;
    ...
    var tr =
        ...
        gene.pathways.push({id: 'KEGG:'+r.pathway_id,
                            label : r.pathway_label,
                            source : { id : resource_id, label: 'KEGG'},
                           });
        return gene;
     ...
     * We do not currently have a resource for gene-pathway publications, but this
     * should work with a few changes once that resource is available
    if (pathList.pathways){
     for (var i=0; i < pathList.pathways.length; i++){
    	pathList.pathways[i].references = [];
    	var gene_id = id.replace(/NCBI_?[Gg]ene:/,"");
    	var path_id = pathList.pathways[i].id.replace(/KEGG:/,"")
    	var filter = ["gene_ids:"+this.quote(gene_id),"pathway_ids:"+this.quote(path_id)];
    	var geneToKOEvidence = this.fetchDataFromResource(null,
    			                                     evidence_map_resource_id,
    			                                     null,
    			                                     null,
    			                                     filter,
    			                                     null,
    			                                     null,
    			                                     {useClosure : false});
    	if (geneToKOEvidence.results){
    		for (var j=0; j < geneToKOEvidence.results.length; j++){
    			var ref = geneToKOEvidence.results[j].publication_ids.split(", ");
    			if (ref){
...
bbop.monarch.Engine.prototype.fetchAssertedDiseasePathwayAssociations = function(id,label) {
    ...
    for (var i=0; i<fetchPath.results.length; i++){
		...
		for (var j=0; j < pathway_ids.length; j++) {
			var pathway = {id : 'KEGG:'+pathway_ids[i].trim(), label : pathway_labels[j].trim()};
			var assoc = {
				id : "monarch:disco/" + r.e_uid,
				type : "Association",
				//TODO: using omim ids instead of disease_id as primary b/c we 
				//haven't mapped kegg diseases into uberdisease
				//also, since omim ids are a list, and it uses transivity to resolve it,
				//but i don't know which one is matched, i will just use the query 
				...
				references : [{id : 'KEGG:'+r.disease_id, label : r.disease_label}]
			};
...
/*
 * given a set of genes (in the gene_assoc list), fetch the pathways, and infer that the 
 * diseases are linked to the pathways via the genes
 */
...
bbop.monarch.Engine.prototype.annotateText = function(txt, opts) {
    var q = opts;
    q.content = txt;
    var excludeCategories = opts.excludeCategories;
    if (excludeCategories == null) {
        excludeCategories = ['resource'];
    }
    var res = 
        this.fetchUrl("http://neuinfo.org/servicesv1/v1/annotate/complete.json", q); // TODO - config
    var results = JSON.parse(res);
    results.entities.forEach(function(r) {
        var cat = r.entity.category == null ? "" : r.entity.category.toLowerCase();
        if (/anatomical/.test(cat)) {
            cat = 'anatomy';
        }
        else if (/cell/.test(cat)) {
            cat = 'anatomy';
        }
        r.entity.category = cat;
    });
    var dupeMap = {};
    results.entities = results.entities.filter(function(r) {
        var e = r.entity;
        var isDupe = dupeMap[e.id];
        dupeMap[e.id] = true;
        return !isDupe;
    });
    results.entities = results.entities.filter(function(r) {
        var c = r.entity.category;
        if (excludeCategories.indexOf(c) == -1) {
            return true;
        }
        return false;
    });
...
bbop.monarch.Engine.prototype.fetchUrl = function(url, params, method) {
    ...
    console.log("Status != 200. ExchangeObj source= "+exchangeObj.toSource());
        throw({
            type : "fetchUrl",
            url: url,
