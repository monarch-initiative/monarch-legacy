...
bbop.monarch.Engine.prototype.fetchClassInfo = function(id, opts) {
    ...
    var res = engine.getVocabularyByID(id);
    //TODO do we need to handle lists of >1
    resultObj = {};
    if (res.list instanceof Array && res.list[0]){
    ...
    if (opts != null) {
        if (opts.level != null && opts.level > 0) {
            ...
            if (rel != null && rel instanceof Array && rel.path[0]){   
    ...
    for (var i =0; i < results.length; i++) {
        ...
        for (var j =0; j < filtered.length; j++) {
             if (results[i].id === filtered[j].id || 
                 ...
                 results[i].label.category === filtered[j].label.category)) 
            {
                found = true;
                break;
            }
...
bbop.monarch.Engine.prototype.searchOverOntologies = function(term, opts) {
    ...
    results.forEach(function(r,i,list){
        if (r.curie != null){
            list[i].id = r.curie;
        } else if (r.fragment != null){
            list[i].id = r.fragment;
        } 
    });
    return results;
}
...
bbop.monarch.Engine.prototype.getVocabularyByTerm = function(term,searchType,limit,category,searchSynonyms,ontology,trFunction) {
    ...
    var resultObj = engine.querySciGraph(uri,params,trFunction);
    return resultObj;
}
