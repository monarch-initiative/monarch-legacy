...
bbop.monarch.Engine.prototype.fetchDiseaseInfo = function(id, opts) {
    ...
    for (var i=0; i<disease_ids.length; i++) {
		...
		//TODO fetchInferredDiseasePathwayAssociations(id,gene_associations) - by genes
    }
...
bbop.monarch.Engine.prototype.fetchPathwaysForDisease = function(id) {
    ...
    var tr =
        function (r) {
			...
			var references = {id : 'KEGG:'+r.disease_id.replace(/:/,'_'), label : r.disease_label};
			obj = {
				id : id,   //disease id
				genes : genes,
				pathways : pathways,
...
bbop.monarch.Engine.prototype.fetchAssertedDiseasePathwayAssociations = function(id,label) {
    ...
    for (var i=0; i<fetchPath.results.length; i++){
		...
		for (var j=0; j < pathway_ids.length; j++) {
			var pathway = {id : 'KEGG:'+pathway_ids[i].trim().replace(/:/,'_'), label : pathway_labels[j].trim()};
			var assoc = {
				id : "monarch:disco/" + r.e_uid,
				type : "Association",
				...
				references : [{id : 'KEGG:'+r.disease_id.replace(/:/,'_'), label : r.disease_label}]
			};
