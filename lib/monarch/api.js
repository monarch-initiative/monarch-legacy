...
bbop.monarch.Engine.prototype.fetchDiseaseInfo = function(id, opts) {
    ...
    for (var i=0; i<disease_ids.length; i++) {
        ...
        /* This searches the phenotype_associations object for publications to be
         * added to the literature tab. */
        ...
        /* This searches the gene_associations object for publications to be
         * added to the literature tab. */
        ...
        /* This searches the alleles object for publications to be added to the
         * literature tab. */
        if (alleles != null) {
...
// var categories = getConfig('summary_categories');
	...
	//this.log("ANNOTATION_SUFFICIENCY: "+info_profile.scaled_score);
...
bbop.monarch.Engine.prototype.fetchPhenotypeInfo = function(id, opts) {
    ...
    obj.genotype_associations = obj.genotype_associations.concat(this.fetchGenoPhenoAsAssociationsBySpecies(id,'7227','phenotype').results); // Fly

    /* This searches the disease_associations object for publications to be
     * added to the literature tab. */
    ...
    /* This searches the gene_associations object for publications to be added to
     * the literature tab. */
    if (obj.gene_associations != null) {
        for (var i = 0; i < obj.gene_associations.length; i += 1) {
            var gene = obj.gene_associations[i].gene;
            var source = obj.gene_associations[i].source;
            var ref = obj.gene_associations[i].references;
            if (ref != null) {
                for (var j = 0; j < ref.length; j += 1) {
                    var regex = /^(PMID|pmid):(\d+)$/;
                    var regres = regex.exec(ref[j].id);
    ...
    /* This searches the genotype_associations object for publications to be
     * added to the literature tab. */
...
bbop.monarch.Engine.prototype.makeInferredGenePhenotypeAssociations = function(assocs) {
    ...
    if (assocs != null) {
        for (var i = 0; i < assocs.length; i += 1) {
            ...
            if (assoc.has_genotype != null && typeof assoc.has_genotype != 'undefined') {
...
bbop.monarch.Engine.prototype.fetchGenotypeInfo = function(id, opts) {
    ...
    var obj = this.fetchGenotype(id,'10090'); // Mouse
        // this.log("mouseGENO:"+JSON.stringify(obj));
    if (obj == null) {
        var obj = this.fetchGenotype(id,'7955'); // Fish
    } else {
        this.log("mouse genotype found");
    }
    if (obj == null) {
        var obj = this.fetchGenotype(id,'6239'); // Worm
    } else {
        this.log("worm genotype found");
    }
    if (obj == null) {
        var obj = this.fetchGenotype(id,'9606'); // Human
    } else {
        this.log("human genotype found");
    }
	if (obj == null) {
		var obj = this.fetchGenotype(id,'7227'); //Fly
	} else {
		this.log("fly genotype found")
	}

    // this.log("GENO:"+JSON.stringify(obj));
    ...
    obj.phenotype_associations = obj.phenotype_associations.concat(this.fetchGenoPhenoAsAssociationsBySpecies(id,'7227').results); // Fly
...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
    ...
    //Add API version
    gene.apiVersionInfo = this.apiVersionInfo();
    ...
    if (obj.source == "NOT FOUND") {
        ...
        this.log(JSON.stringify(mappings));
    ...
    this.log("GENE: "+JSON.stringify(gene));
    ...
    if (genePathways == null || genePathways.pathways == null) {
        this.warn("No pathway info for "+gene.id);
    }
    else {
    ...
    if ((!gene.disease_associations[0])&&(gene.orthologs)&&(gene.taxon)&&(!gene.taxon.id.match(/9606$/))){
    ...
    /* This searches the interactions object for publications to be added to the
     * literature tab. */
...
bbop.monarch.Engine.prototype.fetchModelOrgGenePhenotypeAsAssociation = function(id,source,genObj) {
    ...
    var source_to_resource_map = {    // HARDCODE ALERT
            'MGI'  : {id:'nif-0000-00096-6',label:'MGI',annotation_entity:'effective_genotype'},   // MGI
            'ZFIN' : {id:'nif-0000-21427-10',label:'ZFIN',annotation_entity:'effective_genotype'}, // ZFIN
            'WB'   : {id:'nif-0000-00053-4',label:'WB',annotation_entity:'sequence_alteration'},    // WB
        };
    // TODO integrate flybase data
	resource = source_to_resource_map[source];
...
bbop.monarch.Engine.prototype.fetchDiseasePhenotypeAsGeneAssocations = function(id,genObj) {
    ...
    //this.log("OBJ="+JSON.stringify(id));
    if (!id){
        return{};
    }
...
bbop.monarch.Engine.prototype.fetchOmimDiseasePhenotypeAsAssocations = function(id, opts) {
	...
	subclassFilters['phenotype_id'] = id;
        ...
        this.warn("Assuming ID is a phenotype: "+id);
	subclassFilters['phenotype_id'] = id;
    }
    filters["aspect"] = "O";
...
//    this.log("CACHE " + JSON.stringify(obj, null, ' '));
    return obj;
};
...
bbop.monarch.Engine.prototype.fetchGenotype = function(id,sp) {
    ...
    // this.log("GENOLookup: "+JSON.stringify(geno));
...
// '7955' : {id:'',label'zebrafish models'}
    ...
    if (typeof target_species == 'undefined') {
        this.warn("ERROR: no target_species supplied. Defaulting to human, 9606.");
...
bbop.monarch.Engine.prototype.searchByPhenotypeProfile = function(query,target_species,target_type,cutoff,metric) {
    ...
    // rank is based on rank returned by owlsim, currently hardcoded to be combinedScore
     ...
     //   this.log("taxon match for id"+r.j.id+"="+JSON.stringify(taxon));
    ...
    //if (typeof target_species == 'undefined') {
    //  this.log('SIMILAR:'+JSON.stringify(similarThings,null,' '));
    //}
    return similarThings;
 ...
 */
bbop.monarch.Engine.prototype.generateHistogram = function(id,levels,isCached,filter) {
	//Check cache
	if (this.cache != null) {
        var cached = this.cache.fetch('stats', 'summary-stats');
        if (cached != null) {
            if (cached.apiVersionInfo == null || cached.apiVersionInfo != this.apiVersionInfo()) {
                this.log("cached version is out of date - will not use");
            }
    ...
    for (var k in graph.relationships.relationship) {
        ...
        hp_term = hp_term.replace(/_/,':');
        hp_labels[hp_term]= hp_label;
    }

    //var urlBase = "http://nif-services.neuinfo.org/servicesv1/v1/federation/data/";
    //var opts = "&includeSubclasses=true&exportType=data";
    var groups = [];
    resources.forEach(function(r){groups.push(r.label);});
    summary_statistics.groups = groups;
    
    Object.keys(hp_labels).forEach(function(category) {
    	var group = {};
    	group.counts = [];
    	group.phenotype = hp_labels[category];
    	//Reformat labels
    	group.phenotype = group.phenotype.replace(/Abnormality of (the )?/,"");
    	var capLabel = group.phenotype.replace(/\b[a-z]/g, function() {
    		           return arguments[0].toUpperCase()});
    	group.phenotype = capLabel;
    	group.id = category;
        ...
        if (levels > 0){
        	...
        	var subGraph = engine.generateHistogram(category,levels);
    ...
    if ((this.cache != null) && (isCached)) {
        this.cache.store('stats', 'summary-stats',summary_statistics);
    }
...
bbop.monarch.Engine.prototype.fetchClassInfo = function(id, opts) {
	...
	var tr = function (r) {
        ...
        var obj = {
            ...
            'has_exact_synonym' : r.synonyms,
            'database_cross_reference' : []
	    }; 
    	return obj;
    };
    
	var res = this.getVocabularyByID(id,tr);
    ...
    if (opts != null) {
        if (opts.level != null && opts.level > 0) {
        	tr = function (r) {
        		...
        		if (r.edges[0]){
        			r.edges.forEach(function(k) {
        			    ...
        			    //Get subject and object labels from nodes
        			    r.nodes.forEach(function(i) {
        			        if (triple.subject.id == i.id){
        			            triple.subject.label = i.lbl;
        			        } else if (triple.object.id == i.id){
        			            triple.object.label = i.lbl;
    			            }
        			    });
        			});
                }
            ...
            var rel = this.getGraphNeighbors(id,opts.level,tr);
            resultObj.relationships = rel;
        }
    }
...
/* Deprecated fetchClassInfo using ontoquest
bbop.monarch.Engine.prototype.fetchClassInfo = function(id, opts) {
    var nif_id = this.getOntoquestNifId(id);
	var xmlStr = "";
	try {
        ...
        if (opts.level != null && opts.level > 0) {
           ...
           // this.log(xmlStr);
            ...
            for (var k in g.relationships.relationship) {
				...
				this.log("Erorr when fetching class info; continuing; \n"+err);
			}
...
bbop.monarch.Engine.prototype.searchSubstring = function(term, opts) {
    ...
    var tr = function (r) {
    	...
    	if (r.concept.uri.match(/NCBIGene/)&&(!obj.category)){
    		obj.category = 'gene';
    	}
    ...
    var results = this.getVocabularyByTerm(term,searchType,limit,null,null,null,tr);
...
bbop.monarch.Engine.prototype.searchOverOntologies = function(term, opts) {
    ...
    var tr = function (r) {
    	...
    	if (obj.id.match(/NCBIGene/)&&(!obj.category)){
    		obj.category = 'gene';
    	} else if (obj.id.match(/DOID/)&&(!obj.category)){
    ...
    results = this.getVocabularyByTerm(term,searchType,limit,null,null,null,tr);
...
bbop.monarch.Engine.prototype.fetchUrl = function(url, params, method) {
    ...
    if (exchangeObj.status != 200) {
        // this.log("Status != 200. ExchangeObj source=
...
//    if (cls.id.indexOf("DOID") == 0) {
        ...
        for (var k in altcls.relationships) {
            ...
            if (rel.property.id == "EQUIVALENT_TO") {
...
//
bbop.monarch.Engine.prototype.getGraphNeighbors = function(id,depth,trFunction) {
    ...
    resultObj = this.querySciGraph(uri,params,trFunction);
    return resultObj;
}
...
bbop.monarch.Engine.prototype.getVocabularyByID = function(id,trFunction) {
...
bbop.monarch.Engine.prototype.getVocabularyByTerm = function(term,searchType,limit,category,searchSynonyms,ontology,trFunction) {
    ...
    } else if (searchType.match(/prefix/i)){
    	path = path + 'prefix/'+term+'.json';
    }
...
bbop.monarch.Engine.prototype.querySciGraph = function(uri,params,trFunction) {
	...
	//Fetch URI
    try {
        resultStr = this.fetchUrl(uri,params);
        sciObj = JSON.parse(resultStr);
    }
    catch(err) {
        return errorResponse(err);
    }
    if (trFunction){
        if (sciObj instanceof Array){
            sciObj.forEach(function(r) {
                var result = trFunction(r);
                if (result){
                    resultObj.push(result);
                }
            });
        } else {
    	    sciObj.list.forEach(function(r) {
                var result = trFunction(r);
                if (result){
                    resultObj.push(result);
                }
