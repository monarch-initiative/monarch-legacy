...
bbop.monarch.Engine.prototype.searchOverOntologies = function(term, opts) {
    ...
    if (/\S+:\S+/.test(term)) {
        resultObj = engine.getVocabularyByID(term);
    } else {
        resultObj = engine.getVocabularyByTerm(term,searchType,limit,categories,searchSynonyms);
    }
    ...
    if (resultObj.concepts instanceof Array){
        results = resultObj.concepts;
    }
