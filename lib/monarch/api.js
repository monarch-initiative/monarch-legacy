...
bbop.monarch.Engine.prototype.fetchGeneInfo = function(id, opts) {
    ...
    for (var i=0;i < disease_associations.length; i++){
    	if (disease_associations[i].gene.id == gene.id){
    		gene_disease_associations = 
    			gene_disease_associations.concat(disease_associations[i])
    	}
    }
    disease_associations = gene_disease_associations;
...
bbop.monarch.Engine.prototype.fetchOmimDiseasePhenotypeAsAssocations = function(id) {
    ...
    var resultObj = 
        this.fetchDataFromResource(this.quote(id), 
                resource_id,
                trOmim
                );
...
bbop.monarch.Engine.prototype.fetchPubFromPMID = function(ids) {
    ...
    var totalInfo = [];
    while (ids.length > 0) {
        var query = ids.splice(0, 10).join("&pmid=");
        var resultStr = this.fetchUrl(urlBase + "pmid.json?pmid=" + query);
