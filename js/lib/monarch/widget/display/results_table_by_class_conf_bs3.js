...
function InitMonarchBBOPWidgetDisplay() {
    ...
    if (typeof bbop.monarch.widget.display == "undefined") {
        bbop.monarch.widget.display = {};
    }

    /*
     * Function: results_table_by_class_conf_bs3
     * 
     * Using a conf class and a set of data, automatically populate and return a
     * results table.
     * 
     * This is the Bootstrap 3 version of this display. It affixes itself
     * directly to the DOM using jQuery at elt_id.
     * 
     * Parameters: class_conf - a <bbop.golr.conf_class> golr_resp - a
     * <bbop.golr.response> linker - a linker object; see <amigo.linker> for
     * more details handler - a handler object; see <amigo.handler> for more
     * details elt_id - the element id to attach it to selectable_p -
     * *[optional]* whether to create checkboxes (default true)
     * 
     * Returns: this object
     * 
     * See Also: <bbop.widget.display.results_table_by_class>
     */
    bbop.monarch.widget.display.results_table_by_class_conf_bs3 = function(
            cclass, golr_resp, linker, handler, elt_id, selectable_p,
            select_toggle_id, select_item_name) {
            ...
            if (retval.length > (trimit + 50)) {
                ...
                if (!anchors_p && !list_p && !space_p) {
                    ...
                    if (new_str_list.length <= 3) {
                        ...
                        new_str = new_str + '<br />';
                        new_str = new_str + new_str_list.shift();
                        tease = new bbop.html.span(new_str, {
                            'generate_id' : true
                        });
                    }
                }
                ...
                if (tease) {
                    ...
                    function bgen(lbl, dsc) {
                        var b = new bbop.html.button(lbl, {
                            'generate_id' : true,
                            'type' : 'button',
                            'title' : dsc || lbl,
                    ...
                    trim_hash[tease_id] = [ tease_id, more_b_id, full_id,
                            less_b_id ];

                    // New final string.
                    retval = tease.to_string() + " " + more_b.to_string() + " "
                            + full.to_string() + " " + less_b.to_string();
                }
            }
            ...
            each(headers, function(fid) {
                ...
                if (fid == select_toggle_id) {
                    ...
                    each(bits, function(bit) {
                        var out = anchor.process_entry(bit, fid, doc,
                                display_context);
                        tmp_buff.push(out);
                    });
     ...
     */
    bbop.monarch.widget.display.results_table_by_class_conf_bs3.prototype.process_entry = function(
            bit, field_id, document, display_context) {
